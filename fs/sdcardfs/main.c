...
static const match_table_t sdcardfs_tokens = {
	...
	#ifdef VENDOR_EDIT
	// liangkun@Swdp.shanghai, 2016/02/25, add mount layout_level
	{Opt_layout_level, "layout_level=%u"},
	#endif
	{Opt_err, NULL}
};
				...
				int *debug, struct sdcardfs_mount_options *opts)
{
	...
	#ifdef VENDOR_EDIT
	// liangkun@Swdp.shanghai, 2016/02/25, add mount layout_level
	/* by default, internal storage is PERM_LEGACY_PRE_ROOT */
	opts->layout_level = PERM_LEGACY_PRE_ROOT;
	#endif
	...
	*debug = 0;
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		#ifdef VENDOR_EDIT
		// liangkun@Swdp.shanghai, 2016/02/25, add mount layout_level
		case Opt_layout_level:
			if (match_int(&args[0], &option))
				return 0;
			opts->layout_level= option;
			break;
		#endif
		/* unknown option */
		default:
	...
	if (!err) {
		...
		switch(sb_info->options.derive) {
			...
			case DERIVE_LEGACY:
				...
				#ifdef VENDOR_EDIT
				// liangkun@Swdp.shanghai, 2016/02/25, add mount layout_level
				layout_level = sb_info->options.layout_level;
				if (layout_level < PERM_LEGACY_PRE_ROOT || layout_level > PERM_ROOT) {
					printk(KERN_ERR "sdcardfs: layout_level=%d is invalid, default=%d\n"
							, layout_level, PERM_LEGACY_PRE_ROOT);
					layout_level = PERM_LEGACY_PRE_ROOT;
				}
				setup_derived_state(sb->s_root->d_inode,
						layout_level, 0, AID_ROOT, AID_SDCARD_R, 00771);
				#else
				setup_derived_state(sb->s_root->d_inode,
