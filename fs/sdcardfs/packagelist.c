 ...
 * -------------- Revision History: -----------------
   ...
   Jingchun,Wang	Apr 15,2016	1.1	open sdcard_rw for media
   Jingchun,Wang	May 25,2016	1.2	open sdcard_rw for gps
 *
...
#define AID_GPS         1021  /* gps process */
...
int get_caller_has_rw_locked(void *pkgl_id, derive_t derive) {
	...
	if(AID_MEDIA == (int)appid)
		ret = 1;
	if(AID_GPS == (int)appid)
		ret = 1;
