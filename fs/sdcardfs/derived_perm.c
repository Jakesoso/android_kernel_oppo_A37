...
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
	...
	if (ci->under_dcim && !ci->dcim_uid) {
		ci->dcim_uid = from_kuid(&init_user_ns, current_uid());
	} else {
		ci->dcim_uid = 0;
	}
#endif /* VENDOR_EDIT */
...
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
		case PERM_DCIM:
			if (sbi->options.reserved_mb && !strcasecmp(dentry->d_name.name, "Camera")) {
				info->perm = PERM_DCIM;
				info->under_dcim = true;
			} else if (sbi->options.reserved_mb && !strcasecmp(dentry->d_name.name, "Screenshots")) {
				info->perm = PERM_DCIM;
				info->under_dcim = true;
			}
			break;
