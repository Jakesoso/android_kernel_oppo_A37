...
int dcim_delete_skip(void)
{
	...
	//system app, do skip
	if (from_kuid(&init_user_ns, current_uid()) < AID_APP_START)
		return 0;
	for (i = 0; i < MAX_SKIPD_UID_NUMBER; i++) {
		if (skipd_uid[i] == from_kuid(&init_user_ns, current_uid()))
			return 0;
...
int dcim_delete_uevent(struct dentry *dentry)
{
	...
	if (info->dcim_uid == from_kuid(&init_user_ns, current_uid())) {
	...
	printk("zjm: send delete dcim uevent %s %s, picture uid %u\n", denied_param[1], denied_param[2],
		info->dcim_uid);
