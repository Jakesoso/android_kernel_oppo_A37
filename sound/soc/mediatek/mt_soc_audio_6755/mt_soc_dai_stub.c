...
#ifdef CONFIG_SND_SOC_EXTCODEC_AK4375
/*yongzhi.zhang@Multimedia, 2016/01/05, Add for MT6755*/
extern uint32 mBlockSampleRate[AUDIO_ANALOG_DEVICE_INOUT_MAX];

static int  ak4375_voice_startup(struct snd_pcm_substream *substream,
                                    struct snd_soc_dai *dai)
{
    snd_pcm_hw_constraint_list(substream->runtime, 0,
                               SNDRV_PCM_HW_PARAM_RATE,
...
static int  ak4375_voice_prepare(struct snd_pcm_substream *substream,
                                    struct snd_soc_dai *dai)
{
    printk("%s: SNDRV_PCM_STREAM_CAPTURE rate = %d\n", __func__, substream->runtime->rate);
...
static struct snd_soc_dai_ops mtk_dai_stub_voice_ops =
{
    .startup    = ak4375_voice_startup,
    .prepare   = ak4375_voice_prepare,
};
/*yongzhi.zhang@Multimedia, 2016/01/05, Add for MT6755 End*/
#endif
