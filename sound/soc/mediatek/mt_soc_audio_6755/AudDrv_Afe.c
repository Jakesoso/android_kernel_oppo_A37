...
//yuanyan@Multimedia, 2016/06/28, Add for speaker noise
#if defined(OPPO_MTK_15131_PROJECT)
	APLL_CFG_ADDRESS = ioremap_nocache(APMIXED_CLKCFG_PHYSICAL_BASE, 0x1000);
#endif
}
...
#ifdef VENDOR_EDIT
/*yongzhi.zhang@Multimedia, 2015/11/13, add for MT6755 debugging*/
	if(offset != 0x3a8)
...
{
	long address = (long)((char *)APLL_CFG_ADDRESS + offset);
	uint32 *value;
	value = (uint32 *)(address);
	pr_warn("GetPLLCfg offset=%x address = %lx value = 0x%x\n", offset, address, *value);
	return *value;
}
