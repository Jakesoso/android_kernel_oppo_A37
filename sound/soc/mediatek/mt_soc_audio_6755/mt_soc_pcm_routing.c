...
#ifdef VENDOR_EDIT
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
#if defined(OPPO_MTK_15131_PROJECT)
#define GPIO_I2S0_DAT_PIN         (GPIO75 | 0x80000000)
/*yuanyan@Multimedia, 2015/03/18, update for 15131 I2S0 data in */
#define GPIO_I2S0_PIN_MODE         GPIO_MODE_04
#else
#define GPIO_I2S0_DAT_PIN         (GPIO7 | 0x80000000)
/*yuanyan@Multimedia, 2015/03/18, update for 15131 I2S0 data in */
#define GPIO_I2S0_PIN_MODE         GPIO_MODE_02
#endif

#define GPIO_I2S1_MCLK_PIN         (GPIO17 | 0x80000000)
#define GPIO_I2S1_BCK_PIN         (GPIO18 | 0x80000000)
#define GPIO_I2S1_WS_PIN         (GPIO19 | 0x80000000)
#define GPIO_I2S1_DAT_PIN         (GPIO20 | 0x80000000)

#define GPIO_I2S3_BCK_PIN         (GPIO26 | 0x80000000)
#define GPIO_I2S3_LRCLK_PIN         (GPIO27 | 0x80000000)
#define GPIO_I2S3_DO_PIN         (GPIO28 | 0x80000000)
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755 End*/
#endif
...
static bool AudioI2S0DISetting = false;

static const char *Audio_I2S3_Setting[] = {"Off", "On"};
static bool AudioI2S3Setting = false;
...
#ifdef VENDOR_EDIT
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
static void Auddrv_I2S0DIGpioSet(void)
{
	mt_set_gpio_dir(GPIO_I2S0_DAT_PIN, GPIO_DIR_IN);
/*yuanyan@Multimedia, 2015/03/18, Add for 15131 I2S0 data in start*/
    mt_set_gpio_mode(GPIO_I2S0_DAT_PIN, GPIO_I2S0_PIN_MODE);
	#if defined(OPPO_MTK_15131_PROJECT)
	mt_set_gpio_pull_select(GPIO_I2S0_DAT_PIN, GPIO_PULL_UP);
	mt_set_gpio_pull_enable(GPIO_I2S0_DAT_PIN, GPIO_PULL_ENABLE);
	#endif
/*yuanyan@Multimedia, 2015/03/18, Add for 15131 I2S0 data in end*/
}

static void Auddrv_I2S0DIGpioReset(void)
{
...
#ifdef VENDOR_EDIT
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
	mt_set_gpio_dir(GPIO_I2S1_BCK_PIN, GPIO_DIR_OUT);
	mt_set_gpio_dir(GPIO_I2S1_DAT_PIN, GPIO_DIR_OUT);
	mt_set_gpio_dir(GPIO_I2S1_MCLK_PIN, GPIO_DIR_OUT);
	mt_set_gpio_dir(GPIO_I2S1_WS_PIN, GPIO_DIR_OUT);

	mt_set_gpio_mode(GPIO_I2S1_BCK_PIN, GPIO_MODE_06);
	mt_set_gpio_mode(GPIO_I2S1_DAT_PIN, GPIO_MODE_06);
	mt_set_gpio_mode(GPIO_I2S1_MCLK_PIN, GPIO_MODE_06);
	mt_set_gpio_mode(GPIO_I2S1_WS_PIN, GPIO_MODE_06);
#else
...
#ifdef VENDOR_EDIT
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
	mt_set_gpio_mode(GPIO_I2S1_BCK_PIN, GPIO_MODE_00);
...
#ifdef VENDOR_EDIT
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
void Auddrv_I2S3GpioSet(void)
{
	mt_set_gpio_dir(GPIO_I2S3_BCK_PIN, GPIO_DIR_OUT);
	mt_set_gpio_dir(GPIO_I2S3_DO_PIN, GPIO_DIR_OUT);
	mt_set_gpio_dir(GPIO_I2S3_LRCLK_PIN, GPIO_DIR_OUT);

	mt_set_gpio_mode(GPIO_I2S3_BCK_PIN, GPIO_MODE_06);
    mt_set_gpio_mode(GPIO_I2S3_DO_PIN, GPIO_MODE_06);
    mt_set_gpio_mode(GPIO_I2S3_LRCLK_PIN, GPIO_MODE_06);
	
	Auddrv_I2S0DIGpioSet();
}

void Auddrv_I2S3GpioReset(void)
{
	mt_set_gpio_mode(GPIO_I2S3_BCK_PIN, GPIO_MODE_00);
    mt_set_gpio_mode(GPIO_I2S3_DO_PIN, GPIO_MODE_00);
    mt_set_gpio_mode(GPIO_I2S3_LRCLK_PIN, GPIO_MODE_00);
	
	Auddrv_I2S0DIGpioReset();
}
...
#ifdef VENDOR_EDIT
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
static int AudioI2S0_Setting_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s()\n", __func__);
	...
	AudioI2S0DISetting = ucontrol->value.enumerated.item[0];
	if (AudioI2S0DISetting == true)
		Auddrv_I2S0DIGpioSet();
	else
		Auddrv_I2S0DIGpioReset();
	return 0;
...
static int AudioI2S0_Setting_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s()\n", __func__);
	ucontrol->value.enumerated.item[0] = AudioI2S0DISetting;
	return 0;
}
...
static int AudioI2S3_Setting_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s()\n", __func__);
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Audio_I2S3_Setting)) {
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
	AudioI2S3Setting = ucontrol->value.enumerated.item[0];
	if (AudioI2S3Setting == true)
		Auddrv_I2S3GpioSet();
	else
		Auddrv_I2S3GpioReset();
	return 0;
}

static int AudioI2S3_Setting_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s()\n", __func__);
	ucontrol->value.enumerated.item[0] = AudioI2S3Setting;
	return 0;
}
...
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Audio_I2S3_Setting), Audio_I2S3_Setting),
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755 End*/
#endif
...
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755*/
	SOC_ENUM_EXT("Audio_I2S0_Setting", Audio_Routing_Enum[9],
		     AudioI2S0_Setting_Get, AudioI2S0_Setting_Set),
	SOC_ENUM_EXT("Audio_I2S3_Setting", Audio_Routing_Enum[10],
		     AudioI2S3_Setting_Get, AudioI2S3_Setting_Set),
/*yongzhi.zhang@Multimedia, 2015/09/15, Add for MT6755 End*/
#endif
