...
extern int mi2s0_voice_hdoutput_control;
#endif
...
//yuanyan@Multimedia, 2016/06/28, Add for speaker noise
#if defined(OPPO_MTK_15131_PROJECT)
	if (mi2s0_voice_hdoutput_control == true) {
		pr_warn("%s mi2s0_voice_hdoutput_control=%d\n", __func__,
...
//yuanyan@Multimedia, 2016/06/28, Add for speaker noise
#if defined(OPPO_MTK_15131_PROJECT)
	if (mi2s0_voice_hdoutput_control == true){
		pr_debug("%s mi2s0_voice_hdoutput_control=%d\n", __func__,
		       mi2s0_voice_hdoutput_control);
		/* here to open APLL */
	...
	/* start I2S DAC out */
	SetI2SDacOut(substream->runtime->rate, mi2s0_voice_hdoutput_control, Soc_Aud_I2S_WLEN_WLEN_16BITS);
#else
	/* start I2S DAC out */
