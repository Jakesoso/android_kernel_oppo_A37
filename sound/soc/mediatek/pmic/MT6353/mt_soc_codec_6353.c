...
#ifdef VENDOR_EDIT
//Jianfeng.Qiu@Swdp.Multimedia, 2016/04/18, Add for ts4621
#if defined(CONFIG_SND_SOC_EXTCODEC_TS4621)
extern void ts4621_amp_on(int on, int gain);
extern u8 ts4621_global_gain_value;
#endif
#endif /* VENDOR_EDIT */
...
#ifndef CONFIG_FPGA_EARLY_PORTING
			if (mClkBufferfromPMIC) {
				/*Ana_Set_Reg(MT6353_DCXO_CW01, 0x0000, 0x8000);  temp disable for bringup */
...
{
#ifdef VENDOR_EDIT
//Jianfeng.Qiu@Swdp.Multimedia, 2016/04/18, Add for use ts4621
#if defined(CONFIG_SND_SOC_EXTCODEC_TS4621)
    //Just return
    return;
#endif
...
{
	pr_warn("%s Mux = %d\n", __func__, Mux);
	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON3, Mux << 1, 0xf << 1);	/* Audio offset trimming buffer mux selection */
}

void setOffsetTrimBufferGain(unsigned int gain)
{
	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON3, gain << 5, 0x3 << 5);	/* Audio offset trimming buffer gain selection */
}
...
void EnableTrimbuffer(bool benable)
{
	if (benable == true) {
		...
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON3, 0x1, 0x1);	/* Audio offset trimming buffer enable */
	} else {
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0000, 0x0001);	/* Disable drivers bias circuit */

		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON3, 0x0, 0x1);	/* Audio offset trimming buffer disable */
	}
}
...
void OpenTrimBufferHardware(bool enable)
{
	...
	if (enable) {
		...
		/* Enable IBIST */
		/* Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0700, 0xffff); //Enable HP & HS drivers bias circuit */
		/* Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x1f00, 0xffff); //Enable HP & HS drivers bias circuit    //trim */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x1f01, 0xffff);
		/* Enable HP & HS drivers bias circuit    //trim buffer enable bit MT6328 */

		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON5, 0x5490, 0xffff);	/* HP/HS ibias & DR bias current optimization */
		udelay(50);
		Ana_Set_Reg(MT6353_ZCD_CON2, 0x0489, 0xffff);	/* Set HPR/HPL gain as -1dB */
		Ana_Set_Reg(MT6353_ZCD_CON3, 0x001F, 0xffff);	/* Set voice gain as minimum (~ -40dB) */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON1, 0x0480, 0xffff);	/* De_OSC of HP and enable output STBENH */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON1, 0x1480, 0xffff);	/* De_OSC of voice, enable output STBENH */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xE090, 0xffff);	/* Enable voice driver */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON1, 0x14A0, 0xffff);	/* Enable pre-charge buffer */
		udelay(50);
		...
		pr_warn("%s false\n", __func__);
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0000, 0xffff);	/* Disable drivers bias circuit */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0000, 0xffff);	/* Disable Audio DAC */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x2AC0, 0xfeeb);	/* Disable AUD_CLK, bit2/4/8 is for ADC, do not set */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON7, 0x0000, 0x8000);	/* Disable NV regulator (-1.5V) */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x0001, 0xfeeb);	/* Disable cap-less LDOs (1.5V) & Disable IBIST */
		TurnOffDacPower();
	}
}
...
bool OpenHeadPhoneImpedanceSetting(bool bEnable)
{
	...
	if (bEnable == true) {
		...
		/* HP output swtich release to normal output */
		/*xiang.fei@Multimedia, 2016/01/20, Delete for audio*/
		#if !defined(OPPO_MTK_MT6750_PROJECT) && !defined(OPPO_MTK_15131_PROJECT)
		HP_Switch_to_Release();
		#endif
		/*xiang.fei@Multimedia, 2016/01/20, Delete for audio end*/
...
static void SetDcCompenSation(void)
{
	...
	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON2, RG_AUDHPLTRIM_VAUDP15 << 3, 0x0078);	/* Trim offset voltage of HPL */
	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON2, RG_AUDHPRTRIM_VAUDP15 << 7, 0x0780);	/* Trim offset voltage of HPR */
	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON2, RG_AUDHPLFINETRIM_VAUDP15 << 12, 0x3000);	/* Fine trim offset voltage of HPL */
	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON2, RG_AUDHPRFINETRIM_VAUDP15 << 14, 0xC000);	/* Fine trim offset voltage of HPR */
#endif
}
...
static void Audio_Amp_Change(int channels, bool enable)
{
	if (enable) {
		...
		if (GetDLStatus() == false) {
			/*xiang.fei@Multimedia, 2016/02/22, Delete for audio(P71)*/
			#if !defined(OPPO_MTK_MT6750_PROJECT) && !defined(OPPO_MTK_15131_PROJECT)
			if (mSwitch_Status == 0)
				HP_Switch_to_Ground();
			#endif
			/*xiang.fei@Multimedia, 2016/02/22, Delete for audio (P71) end*/
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0000, 0xffff);
			/* Disable drivers bias circuit */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0000, 0xffff);
			/* Disable Audio DAC */
...
/*xiang.fei@Multimedia, 2016/01/20, Add for hp*/
#if defined(OPPO_MTK_MT6750_PROJECT) || defined(OPPO_MTK_15131_PROJECT)
static int Audio_HP_SWITCH_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    mutex_lock(&Ana_Ctrl_Mutex);
    pr_warn("%s() HP_SWITCH = %ld\n ", __func__, ucontrol->value.integer.value[0]);
    if (ucontrol->value.integer.value[0] == true) {
        udelay(1000);
        mt_set_gpio_out(GPIO_0_DEPOP_SWITCH, GPIO_OUT_ONE); /* high to enable */
    } else if (ucontrol->value.integer.value[0] == false) {
        mt_set_gpio_out(GPIO_0_DEPOP_SWITCH, GPIO_OUT_ZERO); /* low to disable */
        udelay(1000);
    }
    mutex_unlock(&Ana_Ctrl_Mutex);
...
static int Audio_HP_SWITCH_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
#endif
/*xiang.fei@Multimedia, 2016/01/20, Add for hp end*/

#ifdef VENDOR_EDIT
//Jianfeng.Qiu@Swdp.Multimedia, 2016/04/18, Add for ext headphone pa
static int Ext_HP_SWITCH_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    pr_warn("%s() value = %ld\n ", __func__, ucontrol->value.integer.value[0]);
    ...
    if (ucontrol->value.integer.value[0] == true) {
#if defined(CONFIG_SND_SOC_EXTCODEC_TS4621)
        ts4621_amp_on(1, ts4621_global_gain_value);
#endif
    } else if (ucontrol->value.integer.value[0] == false) {
#if defined(CONFIG_SND_SOC_EXTCODEC_TS4621)
        ts4621_amp_on(0, ts4621_global_gain_value);
#endif
    }
    mutex_unlock(&Ana_Ctrl_Mutex);
...
static int Ext_HP_SWITCH_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    return 0;
}
#endif /* VENDOR_EDIT */
...
#if defined(OPPO_MTK_MT6750_PROJECT) || defined(OPPO_MTK_16021_PROJECT)
    if (enable)
    {
        mt_set_gpio_out(GPIO_0_BOOST_EN, GPIO_OUT_ONE); /* high to enable */
        msleep(1);
        mt_set_gpio_out(GPIO_0_SPK_PA_EN, GPIO_OUT_ONE); /* high to enable */
	}
	else
	{
        mt_set_gpio_out(GPIO_0_SPK_PA_EN, GPIO_OUT_ZERO); /* low to disable */
        msleep(1);
        mt_set_gpio_out(GPIO_0_BOOST_EN, GPIO_OUT_ZERO); /* low to disable */
	}
#else
	if (enable) {
		if (GetDLStatus() == false)
...
static int Lineout_PGAR_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	Ana_Set_Reg(MT6353_ZCD_CON1, index << 7, 0x0f10);
...
#ifdef VENDOR_EDIT
//Jianfeng.Qiu@Swdp.Multimedia, 2016/04/18, Add for ext headphone pa
static const char *ext_hp_text[] = {"Off", "On"};

static const struct soc_enum ext_hp_enum[] = {
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ext_hp_text), ext_hp_text),
};
#endif /* VENDOR_EDIT */
...
static const struct snd_kcontrol_new mt6331_snd_controls[] = {
	...
	/*xiang.fei@Multimedia, 2016/01/20, Add for hp*/
	#if defined(OPPO_MTK_MT6750_PROJECT) || defined(OPPO_MTK_15131_PROJECT)
	SOC_ENUM_EXT("Audio_HP_Switch", Audio_DL_Enum[13], Audio_HP_SWITCH_Get,
		     Audio_HP_SWITCH_Set),
	#endif
	/*xiang.fei@Multimedia, 2016/01/20, Add for hp end*/
#ifdef VENDOR_EDIT
//Jianfeng.Qiu@Swdp.Multimedia, 2016/04/18, Add for ext headphone pa
	SOC_ENUM_EXT("Ext_HP_Switch", ext_hp_enum[0], Ext_HP_SWITCH_Get,
		     Ext_HP_SWITCH_Set),
#endif /* VENDOR_EDIT */
};
	...
	if (enable) {
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				...
				/*xiang.fei@Multimedia, 2016/01/20, Modify for micbias0*/
				#if defined(OPPO_MTK_MT6750_PROJECT) || defined(OPPO_MTK_15131_PROJECT) || defined(OPPO_MTK_16021_PROJECT)
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0201, 0xff09);// 1P9
				#else
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0711, 0xff19);
				#endif
				/*xiang.fei@Multimedia, 2016/01/20, Modify for micbias0 end*/
				/* Enable MICBIAS0, MISBIAS0 = 1P9V,
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 1) {
				...
				SetDCcoupleNP(AUDIO_MIC_BIAS1, mAudio_Analog_Mic1_mode);
				/* micbias1 DCCopuleNP */
				/*yuanyan@Multimedia, 2016/04/07, Modify to correct headset micbias start*/
				#if defined(OPPO_MTK_15131_PROJECT)
				//Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0711, 0xff19);
				#else
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0711, 0xff19);
				#endif
				/*yuanyan@Multimedia, 2016/04/07, Modify to correct headset micbias end*/
...
/*xiang.fei@Multimedia, 2015/12/28, Add for audio*/
    ...
    //hp
    mt_set_gpio_mode(GPIO_0_DEPOP_SWITCH, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_0_DEPOP_SWITCH, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_0_DEPOP_SWITCH, GPIO_OUT_ZERO);

    //speaker
    mt_set_gpio_mode(GPIO_0_BOOST_EN, GPIO_MODE_00);	
    mt_set_gpio_dir(GPIO_0_BOOST_EN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_0_BOOST_EN, GPIO_OUT_ZERO); 

    mt_set_gpio_mode(GPIO_0_SPK_PA_EN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_0_SPK_PA_EN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_0_SPK_PA_EN, GPIO_OUT_ZERO);
#elif defined(OPPO_MTK_15131_PROJECT)
    /*yuanyan@Multimedia, 2016/02/23, add for 15131*/
    mt_set_gpio_mode(GPIO_0_DEPOP_SWITCH, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_0_DEPOP_SWITCH, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_0_DEPOP_SWITCH, GPIO_OUT_ZERO);
#elif defined(OPPO_MTK_16021_PROJECT)
    //Jianfeng.Qiu@Swdp.Multimedia, 2016/04/18, Add for 16021
    //speaker
    mt_set_gpio_mode(GPIO_0_BOOST_EN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_0_BOOST_EN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_0_BOOST_EN, GPIO_OUT_ZERO);
