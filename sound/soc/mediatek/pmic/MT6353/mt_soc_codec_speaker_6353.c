...
void Speaker_ClassD_Open(void)
{
	...
	pr_warn("%s\n", __func__);
	/* spk trim */
	Ana_Set_Reg(MT6353_SPK_CON7, 0x4531, 0xffff);	/* TD1,TD2,TD3 for trim (related with trim waiting time) */
	Ana_Set_Reg(MT6353_SPK_CON0, 0x3008, 0xffff);	/* Set to class D mode, set 0dB amplifier gain,  enable trim function */
	Ana_Set_Reg(MT6353_SPK_CON13, 0x3800, 0xffff);	/* Offset trim RSV bit */
	Ana_Set_Reg(MT6353_SPK_CON2, 0x04A4, 0xffff);	/* Turn on OC function, set SPK PGA in DCC mod */
	...
	Ana_Set_Reg(MT6353_SPK_CON13, 0x2000, 0xffff);	/* Clock from Saw-tooth to Triangular wave */
	...
	Ana_Set_Reg(MT6353_SPK_CON0, 0x3001, 0xffff);	/* Turn on speaker (D mode) set 0dB amplifier gain */
}
...
void Speaker_ClassAB_Open(void)
{
	...
	Ana_Set_Reg(MT6353_SPK_CON2, 0x02A4, 0xffff);	/* Turn on OC function, set SPK PGA in DCC mod */
	...
	Ana_Set_Reg(MT6353_SPK_CON13, 0x0000, 0xffff);	/* Clock from Saw-tooth to Triangular wave */
...
void Speaker_ClassAB_close(void)
{
	...
	Ana_Set_Reg(MT6353_SPK_ANA_CON0, 0x0800, 0xffff);	/* Set 0dB PGA gain(level when trimming) */
...
void Speaker_ReveiverMode_Open(void)
{
	...
	Ana_Set_Reg(MT6353_SPK_CON2, 0x02A4, 0xffff);	/* Turn on OC function, set SPK PGA in DCC mode */
