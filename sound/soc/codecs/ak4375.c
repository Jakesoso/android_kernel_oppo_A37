...
#else
#define AK4375_I2C_BUS_NO (4)
#define ECODEC_SLAVE_ADDR_WRITE 0x20
#define ECODEC_SLAVE_ADDR_READ  0x21
static struct i2c_board_info __initdata  ak4375_dev = {I2C_BOARD_INFO("ak4375", (ECODEC_SLAVE_ADDR_WRITE >> 1))};
#endif
...
static const char *clksrc_select[] = {"MCLK", "BLCK"};
static const char *mclk_select[] = {"f9P6MHZ", "f19P2MHZ", "f12P288MHZ", "f24P576MHZ",
                                    "f12P0MHZ", "f24P0MHZ", "f11P289MHZ", "f22P579MHZ"};
...
static const struct soc_enum ak4375_bitset_enum[] = {
    ...
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(clksrc_select), clksrc_select),
...
struct snd_ctl_elem_value  *ucontrol)
{
    ...
    ucontrol->value.enumerated.item[0] = ak4375->clksrc;

    return 0;
}

static int set_clksrc(
struct snd_kcontrol       *kcontrol,
struct snd_ctl_elem_value  *ucontrol)
{
    struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	ak4375->clksrc = ucontrol->value.enumerated.item[0];
...
struct snd_ctl_elem_value  *ucontrol)
{
    ...
    ucontrol->value.enumerated.item[0] = ak4375->mclk;
...
struct snd_ctl_elem_value  *ucontrol)
{
	...
	ak4375->mclk = ucontrol->value.enumerated.item[0];
...
#ifdef VENDOR_EDIT
/* xiaojun.lv@AudioDrv,2015/07/04,add for control pdn GPIO */
static void pdn_enable(int en)
{
    static int enable_flag = -1;
    if (enable_flag == en) {
        return;
    }
    printk("[AK4375] %s enable:%d \n",__FUNCTION__ , en);
    if (en) {
#ifdef CONFIG_MTK_GPIO
		ak4375_gpio_pdn_set(1);
#else
		gpio_direction_output(ak4375_data->ak4375_reset_gpio, 1);
#endif
        mdelay(1);
    } else {
#ifdef CONFIG_MTK_GPIO
		ak4375_gpio_pdn_set(0);
#else
		gpio_direction_output(ak4375_data->ak4375_reset_gpio, 0);
#endif
		mdelay(1);
    }
}
...
//	SOC_ENUM_EXT("AK4375 Soft Mute Cycle Select", ak4375_bitset_enum[3], get_smtcycle, set_smtcycle),
	SOC_ENUM_EXT("AK4375 CLK Source Select", ak4375_bitset_enum[5], get_clksrc, set_clksrc),
	SOC_ENUM_EXT("AK4375 MCLK Frequency Select", ak4375_bitset_enum[6], get_mclk, set_mclk),
...
static int ak4375_set_pllblock(struct snd_soc_codec *codec, int fs)
{
	...
	if(!ak4375_data->clksrc) 
	{ //MCLK
	    if ( (fs % 8000) == 0 ) //48khz base
    	{
    		switch(ak4375_data->mclk)
    		{
    		case F9P6MHZ:
                PLDbit = 5;
                nRefClk = 9600000 / PLDbit;
                break;
            case F19P2MHZ:
                PLDbit = 10;
                nRefClk = 19200000 / PLDbit;
                break;
            case F12P288MHZ:
                PLDbit = 4;
                nRefClk = 12288000 / PLDbit;
                break;
            case F24P576MHZ:
                PLDbit = 8;
                nRefClk = 24576000 / PLDbit;
                break;
            case F12P0MHZ:
                PLDbit = 25;
                nRefClk = 12000000 / PLDbit;
                break;
            case F24P0MHZ:
                nRefClk = 24000000 / PLDbit;
                PLDbit = 25;
                break;
            default:
                pr_err("%s  should not be here! 48k base", __func__);
                nRefClk = 96000000 / PLDbit;
                PLDbit = 5;
    		}
    	}
    	else //44.1khz base
    	{
            ...
            case F22P579MHZ:
                ...
                break;
            default:
                pr_err("%s should not be here! 44.1k base", __func__);
                PLDbit = 25;
            }
    	}
	}
	else //BCLK
    {
    	...
    	{
        /*xiang.fei@Multimedia, 2015/03/06, Modify for hp output distortion*/
#ifdef CONFIG_MTK_SOUND
    		if ( fs <= 16000 ) {
    			PLDbit = 1;
    		...
    		nRefClk = 32 * fs / PLDbit;
#endif
    	/*xiang.fei@Multimedia, 2015/03/06, Modify for hp output distortion end*/
    	}
    ...
    snd_soc_write(codec, AK4375_0E_PLL_CLK_SOURCE_SELECT, ak4375_data->clksrc); //1(BCLK) 0(MCLK)
...
/*yongzhi.zhang@Multimedia, 2015/12/26, add for receiver*/
#ifdef CONFIG_MTK_SOUND
extern uint32 mBlockSampleRate[AUDIO_ANALOG_DEVICE_INOUT_MAX];
static int ak4375_prepare(struct snd_pcm_substream *substream , struct snd_soc_dai *Daiport)
{
    if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
    {
        printk("%s set up SNDRV_PCM_STREAM_CAPTURE rate = %d\n", __FUNCTION__, substream->runtime->rate);
        mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] = substream->runtime->rate;

    }
    else if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
    {
        printk("%s set up SNDRV_PCM_STREAM_PLAYBACK rate = %d\n", __FUNCTION__, substream->runtime->rate);
        mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] = substream->runtime->rate;
    }
    return 0;
}
#endif
/*yongzhi.zhang@Multimedia, 2015/12/26, add for receiver end*/
...
static int ak4375_init_reg(struct snd_soc_codec *codec)
{
	...
	akdbgprt("\t[AK4375 bias] %s(%d)\n",__FUNCTION__,__LINE__);
		...
		ak4375_writeMask(codec, AK4375_0E_PLL_CLK_SOURCE_SELECT, 0x01, ak4375_data->clksrc);	//PLS=1(BICK)
#endif
...
/* MTK start stream event and dapm before configuring widgets, therefore we have to move some settings to the init function. */
#ifdef CONFIG_MTK_SOUND
	ak4375->clksrc = 1;		//1(BCLK) 0(MCLK)
#endif
...
#ifdef CONFIG_MTK_GPIO
	ak4375_gpio_mode_set();
	ak4375_gpio_ldo_set(1);
	ak4375_gpio_pdn_set(1);
#else
...
#else
	int ret;
	ret = i2c_register_board_info(AK4375_I2C_BUS_NO, &ak4375_dev, 1);
#endif
...
#else
subsys_initcall(ak4375_modinit);
#endif
