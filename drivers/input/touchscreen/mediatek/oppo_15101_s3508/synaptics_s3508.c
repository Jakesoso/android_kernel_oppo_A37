...
#endif
#ifdef DMA_SUPPORT
int i2c_dma_write(struct i2c_client *client, u8 addr, s32 len, u8 *txbuf);
int i2c_dma_read(struct i2c_client *client, u8 addr, s32 len, u8 *txbuf);
struct st_i2c_msgs
{
	struct i2c_msg *msg;
	int count;
...
static void synaptics_software_rezero(struct synaptics_ts_data *ts)
{
	...
	i2c_smbus_write_byte_data_s3508(ts->client, F12_2D_CMD_BASE, 0x01);
	msleep(50);
...
int i2c_dma_read(struct i2c_client *client, u8 addr, s32 len, u8 *rxbuf)
{
	int ret;
	s32 retry = 0;
	u8 buffer[1];
...
static ssize_t synaptics_update_fw_store(struct file *file, const char __user *buf, size_t size, loff_t *lo)
{
	...
	TPD_DEBUG(TPD_DEVICE" synaptics:start update fw , buf = %s , size = %zd \n",buf,size);	
...
//mingqiang.guo for charge pulg in ,open tp Finger Amplitude Thre and Finger Dbounce ,avoid charge disturb
	...
	strcpy(ts->fw_name,"tp/15101_FW_S3508_Samsung.img");	
	strcpy(ts->test_limit_name,"tp/15101_Limit_Samsung.img");		
