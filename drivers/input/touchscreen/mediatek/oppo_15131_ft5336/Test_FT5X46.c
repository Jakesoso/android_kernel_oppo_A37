...
static unsigned int m_ucTestItemCode = 0;
...
boolean FT5X46_StartTest()
{
	...
	printk("enter ft5436::FT5X46_StartTest\n");
	//--------------1. Init part
	InitTest();
	...
	for(iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
	{
			...
			)
		{
			...
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				printk("FT5X46_StartTest::RawDataTest failed!\n");
			}
			else{
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
				printk("FT5X46_StartTest::RawDataTest success!\n");
			}
		}
			...
			)
		{
			...
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				printk("FT5X46_TestItem_SCapRawDataTest::failed!\n");
			}
			else{
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
				printk("FT5X46_TestItem_SCapRawDataTest::pass!\n");
			}
		}
...
unsigned char FT5X46_TestItem_RawDataTest(bool * bTestResult)
{
	...
	if(ReCode != ERROR_CODE_OK)		
	{
		printk("\n\n//FT5X46_TestItem_RawDataTest:: Failed to Enter factory Mode. Error Code: %d", ReCode);
		goto TEST_ERR;
	}
	...
	if (m_bV3TP)
	{
		...
		if(ReCode != ERROR_CODE_OK)		
		{
			printk("\n FT5X46_TestItem_RawDataTest::Read REG_MAPPING_SWITCH error. Error Code: %d\n", ReCode);
			goto TEST_ERR;
		}
	...
	if (g_ScreenSetParam.isNormalize == Auto_Normalize)
	{
		...
		for(iRow = 0; iRow<g_ScreenSetParam.iTxNum; iRow++)
		{
			for(iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
			{
				...
				{
					btmpresult = false;
					printk("rawdata test failure_1. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) \n", \
		...
		if(g_stCfg_FT5X22_BasicThreshold.RawDataTest_SetLowFreq)
		{
			...
			for(iRow = 0; iRow<g_ScreenSetParam.iTxNum; iRow++)
			{
				...
				for(iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
				{
					...
					{
						btmpresult = false;
						printk("rawdata test failure_2. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) \n", \
							iRow+1, iCol+1, iValue, RawDataMin, RawDataMax);
					}
				}
			}
		...
		if ( g_stCfg_FT5X22_BasicThreshold.RawDataTest_SetHighFreq )
		{
			...
			for(iRow = 0; iRow<g_ScreenSetParam.iTxNum; iRow++)
			{
				...
				for(iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
				{
					...
					{
						btmpresult = false;
						printk("rawdata test failure_3. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) \n", \
							iRow+1, iCol+1, iValue, RawDataMin, RawDataMax);
					}
				}
			}
...
unsigned char FT5X46_TestItem_SCapRawDataTest(bool * bTestResult)
{
	...
	{
		*bTestResult = true;
		printk("\n\n ft5436 SCap RawData Test is OK!\n");
	}
	else
	{
		* bTestResult = false;
		printk("\n\nft5436 SCap RawData Test is NG!\n");
	}
	return ReCode;
...
unsigned char ReadRawData(unsigned char Freq, unsigned char LineNum, int ByteNum, int *pRevBuffer)
{
	...
	if(ReCode == ERROR_CODE_OK)
	{
		...
		if(ReCode != ERROR_CODE_OK)
		{	
			printk("ft5436::read rawdata Comm_Base_IIC_IO Failed!1 \n");
			goto READ_ERR;
		}
	}
...
static void MergeAllTestData(void)
{
	...
	printk("[focal] MergeAllTestData::%s lenStoreMsgArea=%d,  lenStoreDataArea = %d\n", __func__, g_lenStoreMsgArea, g_lenStoreDataArea);
	printk("MergeAllTestData end\n");
}
...
static void ShowRawData(void)
{
	...
	for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++)
	{
		printk("\nShowRawData::Tx%2d:  ", iRow+1);
		for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
		{
			printk("ShowRawData::%5d    ", m_RawData[iRow][iCol]);
		}
	}
}
