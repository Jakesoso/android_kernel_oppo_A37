					...
					char *buf)
{
	...
	if (fts_read_reg(client, FTS_REG_FW_VER, &fwver) < 0)
		num_read_chars = snprintf(buf, 1024,"get tp fw version fail!\n");
	else
		num_read_chars = snprintf(buf, 1024, "%02X\n", fwver);
...
static ssize_t fts_debug_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
...
static ssize_t fts_debug_read( struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	//struct i2c_client *client = (struct i2c_client *)fts_proc_entry->data;
	struct i2c_client *client = (struct i2c_client *)ft_client;
	int ret = 0;
