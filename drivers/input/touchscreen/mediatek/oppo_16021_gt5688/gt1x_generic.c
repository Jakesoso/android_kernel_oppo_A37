...
int gt1x_resume(void)
{
    ...
    if (!(atomic_read(&double_enable)) && !gt1x_halt)
        return 0;
#else
    if (!gt1x_halt)
        return 0;
...
s32 gt1x_init(void)
{
	...
	if (ret) {
        ...
        //xiongxing@EXP.BaseDdrv.TP, remove for tp bug
        /*
