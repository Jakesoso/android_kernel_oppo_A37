...
PowerCust PowerCustList = {
	{
	 ...
	 {GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_Low},	/* for AFVDD; */
 	 {CAMERA_SUB_VCAMD_EN_PIN, GPIO_MODE_GPIO, Vol_High},	/* for DVDD of sub; */
	 }
};
...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		if (pinSetIdx == 0) {
			if (PowerCustList.PowerCustInfo[1].Gpio_Pin == GPIO_UNSUPPORTED) {
					PK_DBG("[CAMERA SENSOR] Main camera VCAM_D power on %d Vol_1200=%d",pwInfo.Voltage,Vol_1200);
#ifdef CONFIG_MTK_PMIC_CHIP_MT6353
					if(pwInfo.Voltage == Vol_1200){
						if(pmic_set_register_value(PMIC_RG_VCAMD_CAL,0x1))//-20mv
						{
							PK_ERR("fail to set PMIC_RG_VCAMD_CAL");
							return FALSE;
						}
						if (TRUE !=
							hwPowerOn(pwInfo.PowerType, Vol_1220 * 1000, mode_name)) {
							PK_DBG("[CAMERA SENSOR] Fail to enable digital power 1.22v\n");
							return FALSE;
						}
					} else {
						if (TRUE !=
			...
			if (PowerCustList.PowerCustInfo[4].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mt_set_gpio_mode
				    (PowerCustList.PowerCustInfo[4].Gpio_Pin,
				     PowerCustList.PowerCustInfo[4].Gpio_Mode)) {
					PK_DBG("[CAMERA SENSOR] set gpio mode failed!!\n");
				}
				if (mt_set_gpio_dir(PowerCustList.PowerCustInfo[4].Gpio_Pin, GPIO_DIR_OUT)) {
					PK_DBG("[CAMERA SENSOR] set gpio dir failed!!\n");
				}
				if (mt_set_gpio_out
				    (PowerCustList.PowerCustInfo[4].Gpio_Pin,
				     PowerCustList.PowerCustInfo[4].Voltage)) {
					PK_DBG("[CAMERA SENSOR] set gpio failed!!\n");
				}
			}
		}
	...
	if (pwInfo.PowerType == AVDD) {
		...
		if(pinSetIdx == 0){
			...
			if (PowerCustList.PowerCustInfo[4].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mt_set_gpio_out
				    (PowerCustList.PowerCustInfo[4].Gpio_Pin,
				     1-PowerCustList.PowerCustInfo[4].Voltage)) {
					PK_DBG("[CAMERA LENS] set gpio failed!!\n");
				}
			}
		}
