...
static unsigned int single_little_cpufreq_limit;
#endif
...
static int _restore_default_volt(struct mt_cpu_dvfs *p)
{
	...
	if (!cpu_dvfs_is_availiable(p) || !cpu_dvfs_is_availiable(p_second)) {
		FUNC_EXIT(FUNC_LV_HELP);
		return 0;
	}
			       ...
			       struct cpufreq_policy *policy, unsigned int target_volt)
{
	...
	if (cur_khz != get_turbo_freq(p->cpu_id, target_khz)) {
		cpufreq_dbg
			("@%s(), %s: (%d, %d): target_khz = %d (%d), target_volt = %d (%d), num_online_cpus = %d, cur_khz = %d\n",
...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, 2016/4/11, Add API for single core cpufreq limit
static unsigned int mt_cpufreq_get_single_little_cpufreq_limit(void)
{
	return single_little_cpufreq_limit ?
		single_little_cpufreq_limit : SIGLE_CORE_IDX;
}

int mt_cpufreq_set_single_little_cpufreq_limit(unsigned int cpufreq_limit)
{
	single_little_cpufreq_limit = cpufreq_limit;
	return 0;
}
EXPORT_SYMBOL(mt_cpufreq_set_single_little_cpufreq_limit);
#endif
...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, 2016/4/11, Add API for single core cpufreq limit
		if (online_cpus == 1 && (new_opp_idx > mt_cpufreq_get_single_little_cpufreq_limit()))
			new_opp_idx = mt_cpufreq_get_single_little_cpufreq_limit();
#else
	...
	for_each_cpu(i, &cpu_online_cpumask) {
		...
		if (policy[MT_CPU_DVFS_LITTLE]) {
			...
			if (p->idx_opp_ppm_limit == -1)
				...
				policy[MT_CPU_DVFS_LITTLE]->max = cpu_dvfs_get_freq_by_idx(p, p->idx_opp_ppm_limit);
			if (p->idx_opp_ppm_base == -1)
				policy[MT_CPU_DVFS_LITTLE]->min = cpu_dvfs_get_min_freq(p);
			else
				policy[MT_CPU_DVFS_LITTLE]->min = cpu_dvfs_get_freq_by_idx(p, p->idx_opp_ppm_base);
	...
	for_each_cpu(i, &cpu_online_cpumask) {
		...
		if (policy[MT_CPU_DVFS_BIG]) {
			...
			if (p->idx_opp_ppm_base == -1)
				...
				policy[MT_CPU_DVFS_BIG]->min = cpu_dvfs_get_freq_by_idx(p, p->idx_opp_ppm_base);
...
/* Prepare OPP table for PPM in probe to avoid nested lock */
	for_each_cpu_dvfs(j, p) {
		...
		if (NULL == p->freq_tbl_for_cpufreq) {
			...
			table[opp_tbl_info->size].index = i;
		...
		if (j == MT_CPU_DVFS_LITTLE) {
			if (lv == CPU_LEVEL_0)
				mt_ppm_set_dvfs_table(0, p->freq_tbl_for_cpufreq, opp_tbl_info->size, DVFS_TABLE_TYPE_FY);
			else
				mt_ppm_set_dvfs_table(0, p->freq_tbl_for_cpufreq, opp_tbl_info->size, DVFS_TABLE_TYPE_SB);
		} else {
			if (lv == CPU_LEVEL_0)
				mt_ppm_set_dvfs_table(4, p->freq_tbl_for_cpufreq, opp_tbl_info->size, DVFS_TABLE_TYPE_FY);
			else
				mt_ppm_set_dvfs_table(4, p->freq_tbl_for_cpufreq, opp_tbl_info->size, DVFS_TABLE_TYPE_SB);
		}
	}
...
static ssize_t cpufreq_power_mode_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (sscanf(buf, "%d", &do_power_mode) == 1)
