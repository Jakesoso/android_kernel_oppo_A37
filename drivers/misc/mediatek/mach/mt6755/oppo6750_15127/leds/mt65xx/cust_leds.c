...
int cust_set_backlight(int level, int div)
{
   ...
   // printk("%s YXQ level is %d\n", __func__, level);
	...
	/* liping-m@PhoneSW.Multimedia,2016/03/18 add for CT FLAG */
#ifdef OPPO_CTTEST_FLAG 
		//if(oppo_lcm_is_which == 1){ 
				bkl_level = (int)((level * 4)/5);
		printk("%s liping-m level is %d\n", __func__, bkl_level);
		//}
#endif
...
//#else /*VENDOR_EDIT*/
	{"lcd-backlight",     MT65XX_LED_MODE_CUST_LCM, (long)cust_set_backlight, {0}},
//#endif /*VENDOR_EDIT*/
};
