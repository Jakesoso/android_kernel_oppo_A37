...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, 2016/3/16, Add GPU min/max freq limit for scene requirement.
unsigned int mt_gpufreqs_num = 0;
struct mt_gpufreq_table_info *mt_gpufreqs = NULL;
EXPORT_SYMBOL(mt_gpufreqs_num);
EXPORT_SYMBOL(mt_gpufreqs);
#else
...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, 2016/3/16, Add GPU min/max freq limit for scene requirement.
	if (idx > g_limited_min_id)
		idx = g_limited_min_id;
#endif
...
enum {
	...
	IDX_OC_LIMITED,
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, 2016/3/16, Add GPU min/max limit for scene requirement.
	IDX_SCENE_LIMITED,
#endif
...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, Add GPU min/max freq limit for scene requirement
static unsigned int mt_gpufreq_power_min_limited_index_array[NR_IDX_POWER_MIN_LIMITED] = { INT_MAX };
#endif
...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, Add GPU min/max freq limit for scene requirement
	/* Check highest frequency in all limitation */
	limited_index = mt_gpufreqs_num - 1;
	for (i = 0; i < NR_IDX_POWER_MIN_LIMITED; i++) {
		if (mt_gpufreq_power_min_limited_index_array[i] < limited_index)
			limited_index = mt_gpufreq_power_min_limited_index_array[i];
		gpufreq_dbg("mt_gpufreq_power_min_limited_index_array[%d] = %d\n", i,
			mt_gpufreq_power_min_limited_index_array[i]);
	}
	g_limited_min_id = limited_index;
	gpufreq_dbg("Final limit frequency lower bound to id = %d, frequency = %d\n",
		g_limited_min_id, mt_gpufreqs[g_limited_min_id].gpufreq_khz);
#endif
	return ret;
}
...
int mt_gpufreq_scene_protect(unsigned int min_freq, unsigned int max_freq)
{
	...
	if (min_freq > max_freq) {
		gpufreq_warn("@%s: GPU DVFS invalid input min_freq:%u max_freq:%u\n",
	...
	for (i = 0; i < mt_gpufreqs_num; i++) {
		if (mt_gpufreqs[i].gpufreq_khz <= max_freq && !max_done) {
			mt_gpufreq_power_limited_index_array[IDX_SCENE_LIMITED] = i;
			max_done = true;
		}
		if (mt_gpufreqs[mt_gpufreqs_num - i].gpufreq_khz >= min_freq && !min_done) {
			mt_gpufreq_power_min_limited_index_array[IDX_SCENE_MIN_LIMITED] = mt_gpufreqs_num - i;
			min_done = true;
		}
	}

	if (!max_done)
		mt_gpufreq_power_limited_index_array[IDX_SCENE_LIMITED] = mt_gpufreqs_num - 1;
	if (!min_done)
		mt_gpufreq_power_min_limited_index_array[IDX_SCENE_MIN_LIMITED] = 0;
	if (mt_gpufreq_power_min_limited_index_array[IDX_SCENE_MIN_LIMITED] <
		mt_gpufreq_power_limited_index_array[IDX_SCENE_LIMITED]) {
		mt_gpufreq_power_limited_index_array[IDX_SCENE_LIMITED] =
			mt_gpufreq_power_min_limited_index_array[IDX_SCENE_MIN_LIMITED];
	}
	mt_gpufreq_power_throttle_protect();
...
#ifdef VENDOR_EDIT
//xiaocheng.li@swdp.shanghai, 2016/3/28, Add gpufreq min limit interface
/************************************************
 * return current GPU minimum limit frequency
 *************************************************/
unsigned int mt_gpufreq_get_min_limit_freq(void)
{
        gpufreq_dbg("current GPU min limit freq is %d MHz\n",
                    mt_gpufreqs[g_limited_min_id].gpufreq_khz / 1000);
        return mt_gpufreqs[g_limited_min_id].gpufreq_khz;
}
...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, 2016/3/16, Add GPU min/max freq limit for scene requirement.
	seq_printf(m, "g_limited_min_id = %d\n", g_limited_min_id);
#endif
