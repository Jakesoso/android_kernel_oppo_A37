...
int _ioctl_set_refresh_rate(unsigned long arg)
{
	int ret = 0;
	void __user *argp = (void __user *)arg;
	...
	/* calc fps and vfp */
	//ret = copy_from_user(&refresh_rate, argp, sizeof(refresh_rate));
	//if (ret) {
	//	DISPERR("%s, copy from user failed.\n", __func__);
	//	ret = -EFAULT;
		//goto done;
	//}
	refresh_rate=(unsigned int)arg;
	if(refresh_rate>=40){
	   vfp = _fps_to_vfp((int)refresh_rate);
	   DISPERR("%s, set refresh rate = %d, vfp = %d\n", __func__, refresh_rate, vfp);
	   ret = primary_display_vdo_porch_change(DSI_VFP, vfp);
	}else{
	   DISPERR("%s, set refresh rate = %d, Cannot set this refresh rate\n", __func__, refresh_rate);
	}
done:
	return ret;
}
...
#ifdef VENDOR_EDIT
/* liping-m@PhoneSW.Multimedia, 2016/06/20 Add for MTK dynamic frame rate*/
	case DISP_IOCTL_SET_REFRESH_RATE:
	{
		return _ioctl_set_refresh_rate(arg);
	}
#endif
	case DISP_IOCTL_CREATE_SESSION:
		{
...
#ifdef VENDOR_EDIT
/* liping-m@PhoneSW.Multimedia, 2016/06/20 Add for 16021 MTK dynamic frame rate*/
#if defined(OPPO16021_HX8394_TRULY_HD720_DSI_VDO)
unsigned long lcm_fps = 60;
static ssize_t lcm_fps_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
    printk("%s fps is =%ld\n", __func__, lcm_fps);
    return sprintf(buf, "%ld\n", lcm_fps);
...
static ssize_t lcm_fps_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t num)
{
    int ret;
    ret = kstrtoul(buf, 10, &lcm_fps);
    printk("%s set lcm fps =%ld\n", __func__, lcm_fps);
    //mtk_disp_mgr_ioctl("/sys/devices/virtual/mtk_disp_mgr/mtk_disp_mgr/lcm_fps", DISP_IOCTL_SET_REFRESH_RATE, (int)lcm_fps);
    mtk_disp_mgr_ioctl("/dev/mtk_disp_mgr", DISP_IOCTL_SET_REFRESH_RATE, (int)lcm_fps);
    return num;
}

static DEVICE_ATTR(lcm_fps, 0644, lcm_fps_show, lcm_fps_store);
#endif
#endif /*VENDOR_EDIT*/
...
/* liping-m@PhoneSW.Multimedia, 2016/06/20 Add for 16021 MTK dynamic frame rate*/
#if defined(OPPO16021_HX8394_TRULY_HD720_DSI_VDO)
		ret = device_create_file(dev, &dev_attr_lcm_fps);
		if (ret < 0)
		{
			printk("%s lcm fps device create file failed!\n", __func__);
		}
#endif
#endif /*VENDOR_EDIT*/
	disp_sync_init();
