...
/* inital value: assume 1 degreeC for temp. <=> 1 unit for total_power(0~100) */
struct phpb_param {
	int tt, tp;
	char type[8];
};
...
static void set_adaptive_gpu_power_limit(unsigned int limit)
{
	...
	tscpu_warn("set_adaptive_gpu_power_limit %d, T=%d\n",
...
/*
	tscpu_warn("thermal_atm_t.t_catm_par.K_TT %d\n",thermal_atm_t.t_catm_par.K_TT);
	tscpu_warn("thermal_atm_t.t_catm_par.K_SUM_TT_LOW %d\n",thermal_atm_t.t_catm_par.K_SUM_TT_LOW);
	tscpu_warn("thermal_atm_t.t_catm_par.K_SUM_TT_HIGH %d\n",thermal_atm_t.t_catm_par.K_SUM_TT_HIGH);
	tscpu_warn("thermal_atm_t.t_catm_par.MIN_SUM_TT %d\n",thermal_atm_t.t_catm_par.MIN_SUM_TT);
	tscpu_warn("thermal_atm_t.t_catm_par.MAX_SUM_TT %d\n",thermal_atm_t.t_catm_par.MAX_SUM_TT);
	tscpu_warn("thermal_atm_t.t_catm_par.MIN_TTJ %d\n",thermal_atm_t.t_catm_par.MIN_TTJ);
	tscpu_warn("thermal_atm_t.t_catm_par.CATMP_STEADY_TTJ_DELTA %d\n",
		thermal_atm_t.t_catm_par.CATMP_STEADY_TTJ_DELTA);


	tscpu_warn("thermal_atm_t.t_continuetm_par.STEADY_TARGET_TJ %d\n",
		thermal_atm_t.t_continuetm_par.STEADY_TARGET_TJ);
	tscpu_warn("thermal_atm_t.t_continuetm_par.MAX_TARGET_TJ %d\n",
		thermal_atm_t.t_continuetm_par.MAX_TARGET_TJ);
	tscpu_warn("thermal_atm_t.t_continuetm_par.TRIP_TPCB %d\n",
		thermal_atm_t.t_continuetm_par.TRIP_TPCB);
	tscpu_warn("thermal_atm_t.t_continuetm_par.STEADY_TARGET_TPCB %d\n",
...
static int adjust_gpu_power(int power)
{
	...
	/*	FIXME: debug only, this check should be moved to some setter functions.
		or deleted if we don't want sma_len is changable during runtime */
	/*
	if (gpu_power_sma_len > MAX_GPU_POWER_SMA_LEN)
		gpu_power_sma_len = MAX_GPU_POWER_SMA_LEN;
	*/
	...
	if (total_power <= MINIMUM_TOTAL_POWER) {
		...
		for (; i < Num_of_GPU_OPP; i++) {
			if ((mtk_gpu_power[i].gpufreq_power <= max_allowed_gpu_power) &&
				...
				highest_possible_gpu_power = mtk_gpu_power[i].gpufreq_power + 1; /* choose OPP with power "<=" limit */
	...
	if (9 <= sscanf(desc, "%d %d %d %d %d %d %d %d %d", &i_id, &i_first_step, &i_theta_r, &i_theta_f,
		...
		if (i_id >= 0 && i_id < MAX_CPT_ADAPTIVE_COOLERS) {
			...
			if (i_min_cpu_pwr > 0)
				...
				MINIMUM_CPU_POWERS[i_id] = mt_ppm_thermal_get_min_power() + 1; /* choose OPP with power "<=" limit */
#endif
			else
				aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"tscpu_write_atm_setting", "Wrong thermal policy");
			...
			if (i_max_cpu_pwr > 0)
				...
				MAXIMUM_CPU_POWERS[i_id] = mt_ppm_thermal_get_max_power() + 1; /* choose OPP with power "<=" limit */
#endif
			else
				aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"tscpu_write_atm_setting", "Wrong thermal policy");
			...
			if (i_min_gpu_pwr > 0) {
				int min_gpuopp_power = (int) mt_gpufreq_get_min_power() + 1; /* choose OPP with power "<=" limit */
				MINIMUM_GPU_POWERS[i_id] = MAX(i_min_gpu_pwr, min_gpuopp_power);
			} else if (i_min_gpu_pwr == 0)
				MINIMUM_GPU_POWERS[i_id] = (int) mt_gpufreq_get_min_power() + 1; /* choose OPP with power "<=" limit */
			else
				aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"tscpu_write_atm_setting", "Wrong thermal policy");
			...
			if (i_max_gpu_pwr > 0) {
				...
				MAXIMUM_GPU_POWERS[i_id] = (int) mt_gpufreq_get_max_power() + 1; /* choose OPP with power "<=" limit */
			else
				aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"tscpu_write_atm_setting", "Wrong thermal policy");
