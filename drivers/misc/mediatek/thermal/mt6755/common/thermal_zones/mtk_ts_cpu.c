...
int thermal_5A_limit_H = 84000; /*84C*/
int thermal_5A_limit_L = 81000; /*81C*/
#endif
...
static int tscpu_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	...
	if ((curr_temp > (trip_temp[0] - 15000)) || (curr_temp < -30000) || (curr_temp > 85000)) {
		printk_ratelimited("%d %d CPU_T=%d\n",
			get_adaptive_power_limit(0), get_adaptive_power_limit(1), curr_temp);
	}
	if ((curr_temp > (trip_temp[0] - 15000)) || (curr_temp < -30000) || (curr_temp > 90000)) {
		tscpu_warn("%d %d CPU_T=%d\n",
			get_adaptive_power_limit(0), get_adaptive_power_limit(1), curr_temp);
	}
	temp_temp = curr_temp;
				  ...
				  loff_t *data)
{
	...
	if (sscanf(desc, "%d", &lv_Tj_out_flag) == 1) {
		...
		tscpu_warn("tscpu_write_Tj_out bad argument\n");
	}
	return -EINVAL;
}
				    ...
				    loff_t *data)
{
	...
	tscpu_warn("tscpu_write_GPIO_out bad argument\n");
	return -EINVAL;
}
#endif
					...
					loff_t *data)
{
	...
	tscpu_warn("tscpu_talking_flag_write bad argument\n");
	return -EINVAL;
}
					   ...
					   size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d %d", &lv_thermal_5A_limit_H,&lv_thermal_5A_limit_L,&lv_fast_polling_trip_temp) == 3) {
				    ...
				    loff_t *data)
{
	...
	if (2 <= sscanf(desc, "%d %d", &trip, &factor)) {
		...
		tscpu_warn("tscpu_write_fastpoll bad argument\n");
	return -EINVAL;
}
#endif
			   ...
			   loff_t *data)
{
	...
	if (sscanf
		...
		tscpu_warn("tscpu_write bad argument\n");
	}
	kfree(ptr_mtktscpu_data);
	return -EINVAL;
}
...
static int tscpu_thermal_probe(struct platform_device *dev)
{
	...
	if (IS_ERR(therm_main)) {
		tscpu_warn("cannot get thermal clock.\n");
