...
int lm3630a_setbacklight(unsigned int level)
{
   // printk("%s level is %d\n", __func__, level);
    ...
    if(get_PCB_Version() >= HW_VERSION__13){
        if(level >= 2 && level <= 40){
            //printk("%s lm3630a_setbacklight LCD close CABC level = %d\n", __func__,level);
            LM3630A_Write_Byte(0x01,0x18);
            cabc_set_flag=true;
            level = 2+(level-1)*37/40;
            if(level < 2) level = 2;
        }else if(level > 40 && cabc_set_flag){
            //printk("%s lm3630a_setbacklight LCD open CABC \n", __func__);
