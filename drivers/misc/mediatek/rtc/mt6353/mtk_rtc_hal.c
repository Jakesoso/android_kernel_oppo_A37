...
static void hal_rtc_set_spare_oppo_fg_value(u16 val)
{
	//RTC_AL_DOM bit8~14
	u16 temp;
	...
	val = (val & (RTC_NEW_SPARE1_OPPO_FG_MASK >> RTC_NEW_SPARE1_OPPO_FG_SHIFT)) << RTC_NEW_SPARE1_OPPO_FG_SHIFT;
	temp = (temp & RTC_AL_DOM_MASK) | val;
...
u16 hal_rtc_get_register_status(const char * cmd)
{
	...
	if (!strcmp(cmd, "XTAL")) {
		...
		return al_hou;
#ifdef VENDOR_EDIT /* OPPO 2016-03-22 sjc Add for charging */
	} else if (!strcmp(cmd, "OPPO_FG")) {
		//RTC_AL_DOM bit8~14
		al_dom = rtc_read(RTC_AL_DOM);
		al_dom = (al_dom & RTC_NEW_SPARE1_OPPO_FG_MASK) >> RTC_NEW_SPARE1_OPPO_FG_SHIFT;
		return al_dom;
#endif
...
void hal_rtc_set_register_status(const char * cmd, u16 val)
{
	if (!strcmp(cmd, "FG")) {
		...
		hal_rtc_set_spare_oppo_fg_value(val);
#endif
...
void hal_rtc_mark_mode(const char *cmd)
{
	...
	if (!strcmp(cmd, "recv")) {
		...
		spar0 = rtc_read(RTC_SPAR0) & (~RTC_SPAR0_REBOOT_KERNEL_BOOT);
		rtc_write(RTC_SPAR0	, spar0 | RTC_SPAR0_REBOOT_KERNEL_BOOT);
		spar0 = rtc_read(RTC_SPAR0);
	}
	else if (!strcmp(cmd, "silence")) {
		spar0 = rtc_read(RTC_SPAR0) & (~RTC_SPAR0_SILENCE_BOOT);
		rtc_write(RTC_SPAR0	, spar0 | RTC_SPAR0_SILENCE_BOOT);
	}
	else if (!strcmp(cmd, "mos")) {
		spar0 = rtc_read(RTC_SPAR0) & (~RTC_SPAR0_MOS_BOOT);
		rtc_write(RTC_SPAR0	, spar0 | RTC_SPAR0_MOS_BOOT);
	}
	else if (!strcmp(cmd, "sau")) {
		spar0 = rtc_read(RTC_SPAR0) & (~RTC_SPAR0_SAU_BOOT);
		rtc_write(RTC_SPAR0	, spar0 | RTC_SPAR0_SAU_BOOT);
	}
	#ifdef VENDOR_EDIT                                                                                                                                                                                                                      
	//mingqiang.guo@bsp.boot 2015/10/16, add for reboot meta
	else if (!strcmp(cmd, "meta")) { 
		spar0 = rtc_read(RTC_SPAR0) & (~RTC_SPAR0_META_BOOT);
		rtc_write(RTC_SPAR0	, spar0 | RTC_SPAR0_META_BOOT);
	}
	#endif //VENDOR_EDIT
...
#ifdef VENDOR_EDIT
//rendong.shi@BSP.boot, 2015/01/23, add for kernel panic mode
u16  is_kernel_panic_reboot()
{
     u16 spar0;
	 spar0 = rtc_read(RTC_SPAR0);
	 spar0 = spar0 & RTC_SPAR0_REBOOT_KERNEL_BOOT;
...
void rtc_enable_k_eosc(void)
{
	...
	pmic_config_interface_nolock(PMIC_RG_SRCLKEN_IN0_HW_MODE_ADDR, 1, PMIC_RG_SRCLKEN_IN0_HW_MODE_MASK, PMIC_RG_SRCLKEN_IN0_HW_MODE_SHIFT);
	pmic_config_interface_nolock(PMIC_RG_SRCLKEN_IN1_HW_MODE_ADDR, 1, PMIC_RG_SRCLKEN_IN1_HW_MODE_MASK, PMIC_RG_SRCLKEN_IN1_HW_MODE_SHIFT);
	...
	pmic_config_interface_nolock(PMIC_EOSC_CALI_START_ADDR, 1, PMIC_EOSC_CALI_START_MASK, PMIC_EOSC_CALI_START_SHIFT);
