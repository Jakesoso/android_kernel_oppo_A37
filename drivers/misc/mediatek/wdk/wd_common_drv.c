...
__weak void dump_wdt_kick_info(int *last_kick_cpu, unsigned int *wdt_length_val, unsigned long long *last_kick_time)
{
}
...
static int wdk_last_kick_cpu;
static unsigned long long wdk_last_kick_time;
static unsigned int wdk_wdt_length;
...
void dump_wdk_bind_info(void)
{
	...
	dump_wdt_kick_info(&wdk_last_kick_cpu, &wdk_wdt_length, &wdk_last_kick_time);

	memset(wk_tsk_buf, 0, sizeof(wk_tsk_buf));
	snprintf(wk_tsk_buf, sizeof(wk_tsk_buf),
		"[WDK]KICK_INFO %d, 0x%x, %lld\n", wdk_last_kick_cpu, wdk_wdt_length, wdk_last_kick_time);
	aee_sram_fiq_log(wk_tsk_buf);
