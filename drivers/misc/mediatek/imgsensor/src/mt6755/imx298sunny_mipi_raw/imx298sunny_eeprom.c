...
BYTE imx298sunny_DCC_data[96]= {0};
BYTE imx298sunny_SPC_data[252]= {0};
...
void read_imx298sunny_SPC(BYTE* data){
	int i;
	int addr = 0x0701;
	int size = 252;
	...
	if(!get_done || last_size != size || last_offset != addr) {
		if(!_read_imx298_eeprom(addr, imx298sunny_SPC_data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			return false;
	...
	memcpy(data, imx298sunny_SPC_data , size);
    return true;
}


void read_imx298sunny_DCC( kal_uint16 addr,BYTE* data, kal_uint32 size){
	int i;
	addr = 0x0801;
	size = 96;
	...
	if(!get_done || last_size != size || last_offset != addr) {
		if(!_read_imx298_eeprom(addr, imx298sunny_DCC_data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			return false;
	...
	memcpy(data, imx298sunny_DCC_data , size);
    return true;
}
