...
#include "imx298sunnymipiraw_Sensor.h"
...
extern void read_imx298sunny_SPC( BYTE* data );
extern void read_imx298sunny_DCC( kal_uint16 addr,BYTE* data, kal_uint32 size);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX298SUNNY_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint16 IMX298SUNNYMIPI_sensorGainMapping[IMX298MIPI_MaxGainIndex][2] ={	
	{64 ,1	},
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
static void imx298_set_pd_focus_area(MUINT32 startpos, MUINT32 size)
{
...
static void imx298_apply_SPC(void)
{
	...
	{
		read_imx298sunny_SPC(imx298_SPC_data);
		read_spc_flag = TRUE;
		return;
	}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	{
		if(gain < IMX298SUNNYMIPI_sensorGainMapping[iI][0])
		{                
			return IMX298SUNNYMIPI_sensorGainMapping[iI][1];       
		}
	...
	if(iI != IMX298MIPI_MaxGainIndex)
	{
    	if(gain != IMX298SUNNYMIPI_sensorGainMapping[iI][0])
    	{
        	 LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX298SUNNYMIPI_sensorGainMapping[iI][0]);
    	}
    }
	LOG_INF("exit IMX298MIPIGain2Reg function\n");
    return IMX298SUNNYMIPI_sensorGainMapping[iI-1][1];
}
...
#if 0
//hufeng modiry for patch from yfx
	/***********PD setting************/	          	          
	write_cmos_sensor(0x3103,0x01);//PD_OUT_EN
   ...
   write_cmos_sensor(0x3166,0x01);//flexible window mode 
   /************flexible window*************/
   write_cmos_sensor(0x3110,0x01);//x_start
   write_cmos_sensor(0x3111,0x01);
   write_cmos_sensor(0x3112,0x01);//y_start
   write_cmos_sensor(0x3113,0x01);
   write_cmos_sensor(0x3114,0x01);//x_end
   write_cmos_sensor(0x3115,0x01);
   write_cmos_sensor(0x3116,0x01);//x_end
   write_cmos_sensor(0x3117,0x01);
#endif
...
#ifdef VENDOR_EDIT
/*oppo hufeng 20151119 add for samsung and suny module*/

static kal_uint16 return_vendor_id(void)
{
    kal_uint16 vendorIdL=0;
    kal_uint16 vendorIdH=0;
    kal_uint16 vendorId=0;
    char pu_send_cmd[2] = {(char)(0x0000 >> 8), (char)(0x0000 & 0xFF) };

    iReadRegI2C(pu_send_cmd, 2, (u8*)&vendorIdL, 1, 0xA0);
    pu_send_cmd[0] = (char)(0x0001 >> 8);
    pu_send_cmd[1] = (char)(0x0001 & 0xFF);
    iReadRegI2C(pu_send_cmd, 2, (u8*)&vendorIdH, 1, 0xA0);
    vendorId = (vendorIdL&0x00FF)|(vendorIdH&0xFF00);
    return vendorId;
}
#endif
/*************************************************************************
...
#ifdef VENDOR_EDIT
/*oppo hufeng 20151119 add for samsung and suny module*/
            vendorId = return_vendor_id();
            LOG_INF("imx298 vendorId is %d\n", vendorId);
            if (vendorId >= 0x01&&vendorId<=0x07)
                if(*sensor_id == 0x0298)
                    *sensor_id =*sensor_id|(vendorId<<12);
#endif
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_imx298sunny_DCC((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
            break;
...
// add by yfx
		case SENSOR_FEATURE_SET_IMX298_PDFOCUS_AREA:
            LOG_INF("SENSOR_FEATURE_SET_IMX298_PDFOCUS_AREA Start Pos=%d, Size=%d\n",*feature_data,*(feature_data+1));
            imx298_set_pd_focus_area(*feature_data,*(feature_data+1));
			break;
// end
