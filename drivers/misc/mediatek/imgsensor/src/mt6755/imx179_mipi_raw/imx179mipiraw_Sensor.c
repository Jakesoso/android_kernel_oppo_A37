...
#ifdef VENDOR_EDIT
/* Add by LiuBin for register device info at 20160616 */
static void register_deviceinfo_imx179()
{
   int temp = 0;
   int otp_index = 0;
   int k = 0;
   ...
   if (otp_index == 1)
       module_id = read_cmos_sensor(0x3425);
   else
       module_id = read_cmos_sensor(0x3405);
   
   LOG_INF("module_id = %d \n", module_id);
   register_imgsensor_deviceinfo("Cam_b", DEVICE_VERSION_IMX179, module_id);
}
#endif
