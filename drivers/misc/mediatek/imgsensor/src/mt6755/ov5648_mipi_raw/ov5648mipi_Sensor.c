...
static bool deviceInfo_registered = false; 
#endif
...
#define RG_Ratio_Typical_lens52   0x137
#define BG_Ratio_Typical_lens52   0x157

#define RG_Ratio_Typical_lens52_ST   0x138
#define BG_Ratio_Typical_lens52_ST   0x167
struct otp_struct
{
...
int ov5648_update_otp()
{
    ...
    else if (current_otp.lens_id==0x52){
        LOG_INF("OV5648 lens_id==0x52 ,module_id == 0x%x\n",current_otp.module_integrator_id);
        RG_Ratio_Typical=RG_Ratio_Typical_lens52;
        BG_Ratio_Typical=BG_Ratio_Typical_lens52;
		if (current_otp.module_integrator_id==0x7){
			RG_Ratio_Typical=RG_Ratio_Typical_lens52_ST;
			BG_Ratio_Typical=BG_Ratio_Typical_lens52_ST;
			}
    }
    else {
...
#ifdef VENDOR_EDIT
/* Add by LiuBin for register device info at 20160616 */
static void register_deviceinfo_ov5648()
{
    int temp = 0;
    int otp_index = 0;
	int k = 0;
    ...
    for (k=1;k<=3;k++) {
       temp = check_otp_ov5648(k);
       if (temp == 2) {
          //valid data
          otp_index = k;
          LOG_INF("ov5648 otp valid data index[%d] \n", otp_index);
          break;
       }
    }
    ...
    mDELAY(5);
    if (otp_index == 1)
        module_id = (read_cmos_sensor(0x3d05) & 0x7f);
    else if (otp_index == 2)
        module_id = (read_cmos_sensor(0x3d0e) & 0x7f);
    else if (otp_index == 3)
        module_id = (read_cmos_sensor(0x3d07) & 0x7f);
    LOG_INF("module_id = %d \n", module_id);
    register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_OV5648, module_id);
}
#endif
...
#ifdef VENDOR_EDIT
/* Add by LiuBin for register device info at 20160616 */
                if(deviceInfo_registered == false){
                    register_deviceinfo_ov5648();
                    deviceInfo_registered = true;
                }
#endif
