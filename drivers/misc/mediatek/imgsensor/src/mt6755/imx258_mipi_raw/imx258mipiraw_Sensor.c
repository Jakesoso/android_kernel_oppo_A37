...
extern void setSensorId(int sensorId);
#endif
...
extern void selective_read_eeprom_imx258(kal_uint16 addr, BYTE* data);
		...
		do {
            *sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
			    selective_read_eeprom_imx258(0,data);
				imgsensor_info.module_id = data[0];
#ifdef VENDOR_EDIT
/* Add by LiuBin for register device info at 20160616 */
				if(deviceInfo_register_value==0x00){
					register_imgsensor_deviceinfo("Cam_b", DEVICE_VERSION_IMX258, imgsensor_info.module_id);
					deviceInfo_register_value=0x01;
				}
/*oppo hufeng 20160523 add to speed up open camera time when first open after power on phone*/
				read_otp_static_array();
/*oppo hufeng 20160526 modify as driverIC fae requested to modify dw9714 init setting*/
				setSensorId(IMX258_SENSOR_ID);
#endif

				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x , module id=%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.module_id);
