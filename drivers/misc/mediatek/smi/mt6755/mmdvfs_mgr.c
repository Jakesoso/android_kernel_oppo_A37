...
/* default enabled */
static int g_is_mmdvfs_enabled = 1;
...
{
	int result = 0;
	result = cancel_delayed_work_sync(&g_mmdvfs_cam_work);
	/*
	if (result)
		MMDVFSMSG("Can't cancel the pending work, cancel_delayed_work_sync: %d\n", result);
	*/
}
...
#endif
		{
			mmdvfs_voltage_enum query_voltage = mmdvfs_query(
			cmd->scen, cmd);
			mmdvfs_voltage_enum current_voltage =
...
int mmdvfs_enable(int enable)
{
	g_is_mmdvfs_enabled = enable;
	char *is_enabled = (enable) ? "enabled" : "disabled";
	MMDVFSMSG("MMDVFS is %s", is_enabled);
}
