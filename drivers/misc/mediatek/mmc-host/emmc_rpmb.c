...
static struct rpmb_user rpmb_user_state[MAX_RPMB_USER_NUMBER];
//#endif
...
//tong.han@Bsp.Group.boot,2016-5-4,Add for access rpmb cmd6 error on AC001
		if (part_config&EXT_CSD_PART_CONFIG_ACC_MASK == 0) {
...
static int emmc_rpmb_open_session(void)
{
	...
	do {
		...
		if (mc_ret != MC_DRV_OK) {
			...
			//#ifdef VENDOR_EDIT //Haitao.Zhou@Prd.BaseDrv for RPMB patch 
			msleep(1000);          
			MSG(INFO, "%s, wait for 1 second and then open device\n", __func__); 
			//#endif           
			continue;
		}
...
#endif
	//#ifdef VENDOR_EDIT //Haitao.Zhou@Prd.BaseDrv for RPMB patch 
	struct rpmb_user user;
	//#endif
	
	MSG(INFO, "%s, !!!!!!!!!!!!\n", __func__);

	//#ifdef VENDOR_EDIT //Haitao.Zhou@Prd.BaseDrv for RPMB patch 
	if (cmd == RPMB_IOCTL_PROGRAM_KEY ||
		cmd == RPMB_IOCTL_READ_DATA ||
		cmd == RPMB_IOCTL_WRITE_DATA)
	{	    
...
#endif
//#ifdef VENDOR_EDIT //Haitao.Zhou@Prd.BaseDrv for RPMB patch           
		case RPMB_IOCTL_GET_RPMB_SIZE:
			MSG(INFO, "%s, cmd = RPMB_IOCTL_GET_RPMB_SIZE!!!!!!!!!!!!!!\n", __func__);
			if ((void *)arg == NULL)
			{
				return -EINVAL;
			}

			unsigned int rpmb_sz = card->ext_csd.raw_rpmb_size_mult * 128 * 1024;
			ret = copy_to_user((void *)arg, (void *)&rpmb_sz, sizeof(unsigned int));
			if (ret != 0) {
				MSG(ERR, "%s, ret=%d, line=%d\n", __func__, ret, __LINE__);
			}
			break;

		case RPMB_IOCTL_SET_USER_STAT:
			MSG(INFO, "%s, cmd = RPMB_IOCTL_SET_USER_STAT!!!!!!!!!!!!!!\n", __func__);
			ret = copy_from_user((void *)&user, (void *)arg, sizeof(struct rpmb_user));
			if (ret != 0) {
				MSG(ERR, "%s, ret=%d, line=%d\n", __func__, ret, __LINE__);
				return ret;
			}

			if (user.uid < RPMB_USER_ID_BASE ||
				user.uid >= RPMB_USER_ID_END)
			{
				MSG(ERR, "uid value error\n");
				return -EINVAL;
			}

			if (rpmb_user_state[user.uid - RPMB_USER_ID_BASE].uid != user.uid)
			{
				MSG(ERR, "rpmb_user_state[] array data error.\n");
				return -EFAULT;
			}

			mutex_lock(&rpmb_lock);
			rpmb_user_state[user.uid - RPMB_USER_ID_BASE].sessionid = user.sessionid;
			rpmb_user_state[user.uid - RPMB_USER_ID_BASE].pid = user.pid;
			mutex_unlock(&rpmb_lock);
			MSG(INFO, "uid: 0x%x, sessionid: %d, pid: %d.\n", user.uid,user.sessionid, user.pid);
			break;

		case RPMB_IOCTL_GET_USER_STAT:
			MSG(INFO, "%s, cmd = RPMB_IOCTL_GET_USER_STAT!!!!!!!!!!!!!!\n", __func__);
			ret = copy_from_user((void *)&user, (void *)arg, sizeof(struct rpmb_user));
			if (ret != 0) {
				MSG(ERR, "%s, ret=%d, line=%d\n", __func__, ret, __LINE__);
				return ret;
			...
			user.sessionid = rpmb_user_state[user.uid - RPMB_USER_ID_BASE].sessionid;
			user.pid = rpmb_user_state[user.uid - RPMB_USER_ID_BASE].pid;
			ret =  copy_to_user((void *)arg, (void *)&user, sizeof(struct rpmb_user));
			if (ret != 0) {
				MSG(ERR, "%s, err=%d, line=%d\n", __func__, ret, __LINE__);
			}
			break;
//#endif			
	default:
...
//#ifdef VENDOR_EDIT //Haitao.Zhou@Prd.BaseDrv for RPMB patch 
	//init rpmb user state table
	rpmb_user_state[0].uid = RPMB_USER_ID_BASE;
	rpmb_user_state[0].sessionid = 0;
	rpmb_user_state[0].pid = 0;
	int i;
	for (i = 1; i < MAX_RPMB_USER_NUMBER; i++)
	{
		rpmb_user_state[i].uid = rpmb_user_state[i-1].uid + 1;
		rpmb_user_state[i].sessionid = 0;
		rpmb_user_state[i].pid = 0;
	}
//#endif
