	...
	unsigned long arg)
{
	...
	if ((struct msdc_ioctl *)arg == NULL) {
		...
		case MSDC_SD_POWER_ON:
#ifndef VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,modified 2015.01.08 for kingston 32G SDCARD
			...
			pr_err("sd ioctl power on!!!\n");
			host = msdc_get_host(MSDC_SD, 0, 0);
			mmc_claim_host(host->mmc);//add
			mmc_power_off(host->mmc);//add
			mmc_release_host(host->mmc);//add
			ret = mmc_resume_host(host->mmc);
			break;
#endif/*VENDOR_EDIT*/
