...
#define MMCDBG(format, ...)	printk(KERN_ERR"mmc3416x " format "\n", ## __VA_ARGS__)
#else
...
static int MMCECS_CheckDevice(void)
{
	...
	printk(" mmc3416x check device id = %x\n",buffer[0]);
	printk(" mmc3416xret = %d\n",ret);
	if(ret < 0)
	{
		return ret;
	}
...
static ssize_t store_layout_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(1 == sscanf(buf, "%d", &layout))
	{
		...
		{
			MMCDBG("HWMSEN_GET_CONVERT function error!\r\n");
		}
		else if(!hwmsen_get_convert(data->hw->direction, &data->cvt))
		{
			MMCDBG("invalid layout: %d, restore to %d\n", layout, data->hw->direction);
		}
		else
		{
			MMCDBG("invalid layout: (%d, %d)\n", layout, data->hw->direction);
			hwmsen_get_convert(0, &data->cvt);
		}
	}
	else
	{
		MMCDBG("invalid format = '%s'\n", buf);
	}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if(NULL == obj)
	{
		MMCDBG("mmc3416x_i2c_data is null!!\n");
		return 0;
	}	
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
//		MMCDBG("invalid content: '%s', length = %d\n", buf, count);
	}
...
{ 
    ...
    /*Read initial data*/
		...
		msleep(MMC3416X_DELAY_SET);
		data[0] = MMC3416X_REG_CTRL;
		data[1] = MMC3416X_CTRL_SET;//MMC3416X_CTRL_RESET
		I2C_TxData(data, 2);
		msleep(1);
		data[0] = MMC3416X_REG_CTRL;
		data[1] = 0;
		...
		data_initial[0] = data[1] << 8 | data[0];
		data_initial[1] = data[3] << 8 | data[2];
		data_initial[2] = data[5] << 8 | data[4];
...
*/
			...
			msleep(MMC3416X_DELAY_SET);
			data[0] = MMC3416X_REG_CTRL;
			data[1] = MMC3416X_CTRL_RESET;//MMC3416X_CTRL_SET
			I2C_TxData(data, 2);
			msleep(1);
			data[0] = MMC3416X_REG_CTRL;
			data[1] = 0;
			...
			data_magnet_close[0] = data[1] << 8 | data[0];
			data_magnet_close[1] = data[3] << 8 | data[2];
			data_magnet_close[2] = data[5] << 8 | data[4];
		...
		MMCDBG(KERN_ERR" memsic disstacnce %d, %d, %d\n", data_magnet_close[0]- data_initial[0], data_magnet_close[1]- data_initial[1], data_magnet_close[2]- data_initial[2]);
	...
	{
		MMCDBG("MMC3416X_IOC_READ ST_XYZ failed\n");
		return -EFAULT;
	}

	printk("memsic ST_SYZ=%d\n",data[0]);
	if((data[0]&0x08)==0x08)
	{
...

{
	...
	switch (cmd)
	{
		...
		case MMC31XX_IOC_SET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MMCDBG("MMC3416X_IOC_SET failed\n");
				return -EFAULT;
			}
			...
			data[1] = MMC3416X_CTRL_SET;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG("MMC3416X_IOC_SET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3416X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG("MMC3416X_IOC_SET failed3\n");
				return -EFAULT;
			}
			msleep(MMC3416X_DELAY_SET);
			break;
		...
		case MMC31XX_IOC_RESET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MMCDBG("MMC3416X_IOC_RESET failed\n");
				return -EFAULT;
			}
			/* wait external capacitor charging done for next SET/RESET */
			msleep(MMC3416X_DELAY_RST);
			...
			data[1] = MMC3416X_CTRL_RESET;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG("MMC3416X_IOC_RESET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3416X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	               MMCDBG("MMC3416X_IOC_RESET failed3\n");
				return -EFAULT;
			}
			msleep(1);
			break;
		...
		case MMC31XX_IOC_READ:
			...
			if(I2C_RxData(data, 6) < 0)
			{
				MMCDBG("MMC3416X_IOC_READ failed\n");
				return -EFAULT;
			}
			vec[0] = data[1] << 8 | data[0];
			vec[1] = data[3] << 8 | data[2];
		...
		//	vec[2] = 65536 - vec[2];
			...
			{
				MMCDBG("MMC3416X_IOC_READ: copy to user failed\n");
				return -EFAULT;
			}
			break;
		...
		case MMC31XX_IOC_READXYZ:
			...
			if(copy_to_user(argp, vec, sizeof(vec)))
			{
				MMCDBG("MMC3416X_IOC_READXYZ: copy to user failed\n");
				return -EFAULT;
			}
			break;			
		...
		case MSENSOR_IOCTL_READ_CHIPINFO:
			if(argp == NULL)
			{
				MMCDBG("IO parameter pointer is NULL!\r\n");
				break;
			}
		...
		default:
			MMCDBG("%s not supported = 0x%04x", __FUNCTION__, cmd);
			return -ENOIOCTLCMD;
			break;		
		}
...
static long mmc3416x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
		...
		//MMCDBG("%s cmd = 0x%04x", __FUNCTION__, cmd);
	
		if(!file->f_op || !file->f_op->unlocked_ioctl)
		{
			MMCDBG("file->f_op OR file->f_op->unlocked_ioctl is null!\n");
			return -ENOTTY;
		}
		...
		switch(cmd)
		{
			case COMPAT_MMC31XX_IOC_TM:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC31XX_IOC_TM is failed!\n");
				}
				break;
			...
			case COMPAT_MMC31XX_IOC_SET:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC31XX_IOC_SET is failed!\n");
				}
				break;
			...
			case COMPAT_MMC31XX_IOC_RM:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC31XX_IOC_RM is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_RESET:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC31XX_IOC_RESET is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_RRM:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC31XX_IOC_RRM is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_READ:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC31XX_IOC_READ is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_READXYZ:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC31XX_IOC_READXYZ is failed!\n");
				}
...
/*			case COMPAT_MMC3416X_IOC_READ_REG:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC3524X_IOC_READ_REG is failed!\n");
				}		
				break;
			...
			case COMPAT_MMC3416X_IOC_WRITE_REG:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC3524X_IOC_WRITE_REG is failed!\n");
				}
			...
			case COMPAT_MMC3416X_IOC_READ_REGS:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MMC3524X_IOC_READ_REGS is failed!\n");
				}	
				break; 
*/	
			case COMPAT_ECOMPASS_IOC_GET_DELAY:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_ECOMPASS_IOC_GET_DELAY is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_SET_YPR:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_ECOMPASS_IOC_SET_YPR is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_OPEN_STATUS:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_ECOMPASS_IOC_GET_OPEN_STATUS is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_MFLAG:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_ECOMPASS_IOC_GET_MFLAG is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_OFLAG:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_ECOMPASS_IOC_GET_OFLAG is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_READ_CHIPINFO:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MSENSOR_IOCTL_READ_CHIPINFO is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_READ_SENSORDATA:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MSENSOR_IOCTL_READ_SENSORDATA is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_LAYOUT:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_ECOMPASS_IOC_GET_LAYOUT is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
				...
				if(ret < 0)
				{
					MMCDBG("COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA is failed!\n");
				}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			{
				MMCDBG("Set delay parameter error!\n");
				err = -EINVAL;
			}
			else
			{
		...
		case SENSOR_ENABLE:
			if((buff_in == NULL) || (size_in < sizeof(int)))
			{
				MMCDBG("Enable sensor parameter error!\n");
				err = -EINVAL;
			}
			else
			{
		...
		case SENSOR_GET_DATA:
			...
			{
				MMCDBG("get sensor data parameter error!\n");
				err = -EINVAL;
			}
			else
			{
			...
			break;
		default:
			MMCDBG("msensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			break;
		default:
			MMCDBG("gsensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
...
//modified for mmc3416x----changhua start
	...
	{
		MMCDBG("mmc3416x_device register failed\n");
...
*/
	...
	register_device_proc("Sensor_msensor", MMC3416X_DEV_NAME, "MICROCHIP");
	#endif//VENDOR_EDIT
...
static int mmc3416x_i2c_remove(struct i2c_client *client)
{
	...
	{
		MMCDBG("mmc3416x_delete_attr fail: %d\n", err);
	}
...
static int mmc3416x_local_init(void)
{
	...
	{
		MMCDBG("mmc3416x add driver error\n");
		return -1;
	} 
...
/*	if(platform_driver_register(&mmc_sensor_driver))
	{
		MMCDBG("failed to register driver");
		return -ENODEV;
	}*/
