	...
	for(i = 0; i < MAX_M_V_SENSOR; i++)
	{
	   ...
	   if(!(cxt->active_data_sensor&(0x01<<i)))
	   {
		...
		if(ID_M_V_MAGNETIC ==i)
		{
			...
			#else /*VENDOR_EDIT*/
			while (((cur_ns - m_pre_ns) >= delay_ms*1800000LL)&&(loop_num>0)){
				m_pre_ns += delay_ms*1000000LL;
				mag_data_report(MAGNETIC,cxt->drv_data[i].mag_data.values[0],
					cxt->drv_data[i].mag_data.values[1],
					cxt->drv_data[i].mag_data.values[2],
			...
			if ((cur_ns - m_pre_ns) >= delay_ms*1800000LL){
				MAG_ERR(" the mag data report is late,cur %d,pre %d, diff %d\n",(int)(cur_ns/1000000LL),(int)(m_pre_ns/1000000LL),(int)((cur_ns-m_pre_ns)/1000000LL));
			}
			#endif /*VENDOR_EDIT*/
		...
		if(ID_M_V_ORIENTATION ==i)
		{
			...
			#else /*VENDOR_EDIT*/
			while (((cur_ns - o_pre_ns) >= delay_ms*1800000LL)&&(loop_num_o>0))
			{
				o_pre_ns += delay_ms*1000000LL;
				mag_data_report(ORIENTATION,cxt->drv_data[i].mag_data.values[0],
					cxt->drv_data[i].mag_data.values[1],
			...
			if ((cur_ns - o_pre_ns) >= delay_ms*1800000LL){
				MAG_ERR(" the ori data report is late,cur %d,pre %d, diff %d\n",(int)(cur_ns/1000000LL),(int)(o_pre_ns/1000000LL),(int)((cur_ns-o_pre_ns)/1000000LL));
			}
			#endif /*VENDOR_EDIT*/
