...
/*----------------------------------------------------------------------------*/
#define GME60X_DEBUG		0
#define GME60X_RETRY_COUNT	10
#define GME60X_DEFAULT_DELAY	100
//#define GME_Softgyro		   // enable this if you need use 6D gyro
 ...
 **************************/
     ...
     *********************/
        ...
        #define MAGN_FUN(f)               printk(MAGN_TAG"%s\n", __FUNCTION__)
        #define MAGN_ERR(fmt, args...)    printk(KERN_ERR MAGN_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
        #define MAGN_LOG(fmt, args...)    printk(MAGN_TAG fmt, ##args)
    #else
        #define MAGN_TAG
...
static struct i2c_board_info __initdata i2c_gme60x={ I2C_BOARD_INFO(GME60X_DEV_NAME, (GMA60X_I2C_ADDRESS>>1))};
/*----------------------------------------------------------------------------*/
...
static int gm_probe(struct platform_device *pdev);
...
static void gme60x_power(struct mag_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, GME60X_I2C_NAME)) 
			{
		...
		{
			if(!hwPowerDown(hw->power_id, GME60X_I2C_NAME)) 
			{
...
static int GME_CheckDevice(void)
{
	...
	MAG_ERR(" GLOBALMEMS check device id");
	/* Set measure mode */
	buffer[0] = GME60X_REG_WIA1;
	/* Read data */
	ret = AKI2C_RxData(buffer, 1);
	//MAGN_LOG(" GLOBALMEMS check device id = %x",buffer[0]);
	MAG_ERR(" GLOBALMEMS check device id = %x",buffer[0]);
	MAG_ERR(" ret = %d",ret);
	//MAGN_LOG(" ret = %d",ret);
	if(ret < 0)
	{
...
int FST_GME60X(void)
{
	...
	// TEST
	TEST_DATA(TLIMIT_NO_RST_WIA1_60X,   TLIMIT_TN_RST_WIA1_60X,   (int)i2cData[0],  TLIMIT_LO_RST_WIA1_60X,   TLIMIT_HI_RST_WIA1_60X,   &pf_total);
	TEST_DATA(TLIMIT_NO_RST_WIA2_60X,   TLIMIT_TN_RST_WIA2_60X,   (int)i2cData[1],  TLIMIT_LO_RST_WIA2_60X,   TLIMIT_HI_RST_WIA2_60X,   &pf_total);
	...
	// TEST
	TEST_DATA(TLIMIT_NO_ASAX_60X, TLIMIT_TN_ASAX_60X, asax, TLIMIT_LO_ASAX_60X, TLIMIT_HI_ASAX_60X, &pf_total);
	TEST_DATA(TLIMIT_NO_ASAY_60X, TLIMIT_TN_ASAY_60X, asay, TLIMIT_LO_ASAY_60X, TLIMIT_HI_ASAY_60X, &pf_total);
	TEST_DATA(TLIMIT_NO_ASAZ_60X, TLIMIT_TN_ASAZ_60X, asaz, TLIMIT_LO_ASAZ_60X, TLIMIT_HI_ASAZ_60X, &pf_total);
	...
	// TEST
	i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SNG_ST1_60X,  TLIMIT_TN_SNG_ST1_60X,  (int)i2cData[0], TLIMIT_LO_SNG_ST1_60X,  TLIMIT_HI_SNG_ST1_60X,  &pf_total);

	// TEST
	TEST_DATA(TLIMIT_NO_SNG_HX_60X,   TLIMIT_TN_SNG_HX_60X,   hdata[0],          TLIMIT_LO_SNG_HX_60X,   TLIMIT_HI_SNG_HX_60X,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HY_60X,   TLIMIT_TN_SNG_HY_60X,   hdata[1],          TLIMIT_LO_SNG_HY_60X,   TLIMIT_HI_SNG_HY_60X,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HZ_60X,   TLIMIT_TN_SNG_HZ_60X,   hdata[2],          TLIMIT_LO_SNG_HZ_60X,   TLIMIT_HI_SNG_HZ_60X,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_ST2_60X,  TLIMIT_TN_SNG_ST2_60X,  (int)i2cData[8], TLIMIT_LO_SNG_ST2_60X,  TLIMIT_HI_SNG_ST2_60X,  &pf_total);
	...
	// TEST
	i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SLF_ST1_60X, TLIMIT_TN_SLF_ST1_60X, (int)i2cData[0], TLIMIT_LO_SLF_ST1_60X, TLIMIT_HI_SLF_ST1_60X, &pf_total);
...
static int gme60x_create_attr(struct device_driver *driver) 
{
	...
	{
		if((err = driver_create_file(driver, gme60x_attr_list[idx])))
		{            
	...
	switch (cmd)
	{
	   ...
	   case ECS_IOCTL_RESET:
		...
		case ECS_IOCTL_SET_MODE:
			...
			ret = AKECS_SetMode(mode);  // MATCH command from GMADFS PART
			if(ret < 0)
			{
				return ret;
			}
		...
		case ECS_IOCTL_GET_OPEN_STATUS:
			MAGN_FUN("IOCTL_GET_OPEN_STATUS");
		...
		case ECS_IOCTL_GET_CLOSE_STATUS:
			MAGN_FUN("IOCTL_GET_CLOSE_STATUS");
		...
		case ECS_IOCTL_GET_OSENSOR_STATUS:
			MAGN_FUN("ECS_IOCTL_GET_OSENSOR_STATUS");
	...
	switch (cmd) {
		 ...
		 case COMPAT_ECS_IOCTL_GET_CONF_60X:
		 	ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_CONF_60X,
							(unsigned long)arg32);
			 if (ret){
			 	MAGN_LOG("ECS_IOCTL_GET_CONF_60X unlocked_ioctl failed.");
				return ret;
			 }
...
/*----------------------------------------------------------------------------*/
static struct file_operations gme60x_fops = {
	.owner = THIS_MODULE,
	.open = gme60x_open,
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	if(atomic_read(&data->trace) & GME_FUN_DEBUG)
	{
		MAGN_FUN("gme60x_orientation_operate");
	}	
#endif
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				value = *(int *)buff_in;
			//	MAG_ERR( "gme60x_orientation_operate SENSOR_ENABLE=%d  mEnabled=%d\n",value,mEnabled);
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	if(atomic_read(&data->trace) & GME_FUN_DEBUG)
	{
		MAGN_FUN("gme60x_gyroscope_operate");
	}	
#endif
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				value = *(int *)buff_in;
			//	MAG_ERR( "gme60x_gyroscope_operate SENSOR_ENABLE=%d  mEnabled=%d\n",value,mEnabled);
				if (mEnabled <= 0) {
						    if(value == 1)
						    {
						     atomic_set(&o_flag, 1);
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	if(atomic_read(&data->trace) & GME_FUN_DEBUG)
	{
		MAGN_FUN("gme60x_rotation_vector_operate");
	}	
#endif
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	if(atomic_read(&data->trace) & GME_FUN_DEBUG)
	{
		MAGN_FUN("gme60x_gravity_operate");
	}	
#endif
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	if(atomic_read(&data->trace) & GME_FUN_DEBUG)
	{
		MAGN_FUN("gme60x_linear_accelration_operate");
	}	
#endif
...
/*----------------------------------------------------------------------------*/
static void gme60x_early_suspend(struct early_suspend *h) 
{
	struct gme60x_i2c_data *obj = container_of(h, struct gme60x_i2c_data, early_drv);   
	int err = 0;
	...
	if ((err = AKECS_SetMode(GME60X_MODE_POWERDOWN)) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return;
	}
...
/*----------------------------------------------------------------------------*/
static void gme60x_late_resume(struct early_suspend *h)
{
	struct gme60x_i2c_data *obj = container_of(h, struct gme60x_i2c_data, early_drv);         
	int err;
	...
	if ((err = AKECS_SetMode(GME60X_MODE_SNG_MEASURE)) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return;
		}
}
...
static int gme60x_m_set_delay(u64 ns)
{
    ...
    MAGN_LOG("%s, value=%d , ns=%lld :%d\n", __FUNCTION__, value, ns, __LINE__);
	if(value <= 10)
    {
        gmad_delay = 10;
    }
	...
	MAGN_LOG("%s, gmad_delay=%d :%d\n", __FUNCTION__, gmad_delay, __LINE__);
	return 0;
}
...
static int gme60x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	MAGN_FUN();
	if(!(data = kzalloc(sizeof(struct gme60x_i2c_data), GFP_KERNEL)))
	{
		err = -ENOMEM;
		goto exit;
	}
	...
	if((err = gme60x_create_attr(&(gme60x_init_info.platform_diver_addr->driver))))
	{
		MAG_ERR("create attribute err = %d\n", err);
		goto exit_sysfs_create_group_failed;
	}

	if((err = misc_register(&gme60x_device)))
	{
	...
	data->early_drv.suspend  = gme60x_early_suspend,
	data->early_drv.resume   = gme60x_late_resume,    
...
static int gme60x_i2c_remove(struct i2c_client *client)
{
	...
	if((err = gme60x_delete_attr(&(gme60x_init_info.platform_diver_addr->driver))))
	{
...
static int __init gme60x_init(void)
{
	...
	i2c_register_board_info(hw->i2c_num, &i2c_gme60x, 1);
