...
#define MSE_DBG(fmt, args...)    printk(KERN_ERR MSE_TAG" %s %d : \r\n"fmt, __FUNCTION__, __LINE__, ##args)
#define MSE_VER(fmt, args...)   ((void)0)
#else
#define MSE_FUN(f)				printk(KERN_ERR MSE_TAG" %s\r\n", __FUNCTION__)
#define MSE_ERR(fmt, args...)	printk(KERN_ERR MSE_TAG" %s %d : \r\n"fmt, __FUNCTION__, __LINE__, ##args)
#define MSE_VER(fmt, args...)
#define MSE_DBG(fmt, args...)
...
#if 0
static int ioctl_init(unsigned long args)
{
    /* nothing to do */
    return 0;
}

static int ioctl_read_chipinfo(unsigned long args)
{
    char *p = (char *) args;
...
#endif
static int ioctl_read_sensordata(unsigned long args)
{
    struct geomagnetic_data *data = i2c_get_clientdata(this_client);
    char buf[64], *p;
...
static int ioctl_read_calidata(unsigned long args)
{
    struct geomagnetic_data *data = i2c_get_clientdata(this_client);
    int buf[7], *p, i;
...
#if 0
static long geomagnetic_dev_ioctl(struct file *f, unsigned int cmd, unsigned long args)
{
    int result = 0;
...
static long compat_yamaha530_unlocked_ioctl(struct file *filp, unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA: {
			//MSE_DBG("compat_ion_ioctl : MSENSOR_IOCTL_XXX command is 0x%x\n", cmd);
...
static int yamaha530_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (hwmsen_read_byte(client, 0x80, databuf)) {

		MSE_ERR("read id register err!\n");
		goto err;
	} else {
		MSE_ERR("read id register %02x!\n",databuf[0]);
		if((databuf[0]!=YAS_YAS530_DEVICE_ID)&&(databuf[0]!=YAS_YAS532_DEVICE_ID)){
			MSE_ERR("err id!\n");
			goto err;
		}
	}
    ...
    if (input_data == NULL) {
        rt = -ENOMEM;
        MSE_ERR("geomagnetic_probe: Failed to allocate input_data device\n");
        goto err;
    }
    ...
    if (rt) {
        MSE_ERR("geomagnetic_probe: Unable to register input_data device: %s\n",
               input_data->name);
        goto err;
    }
    data_registered = 1;
	...
	{
		MSE_ERR("yamaha530 create attribute err\n");
		goto  err;
	}
    ...
    rt = sysfs_create_group(&input_data->dev.kobj,
            &geomagnetic_attribute_group);
    if (rt) {
        MSE_ERR("geomagnetic_probe: sysfs_create_group failed[%s]\n",
               input_data->name);
        goto err;
    }
    sysfs_created = 1;
    ...
    if (input_raw == NULL) {
        rt = -ENOMEM;
        MSE_ERR("geomagnetic_probe: Failed to allocate input_raw device\n");
        goto err;
    }
    ...
    if (rt) {
        MSE_ERR("geomagnetic_probe: Unable to register input_raw device: %s\n",
               input_raw->name);
        goto err;
    }
    raw_registered = 1;
    ...
    if (hwdep_driver.get_filter != NULL) {
        if (hwdep_driver.get_filter(&filter) < 0) {
            MSE_ERR("hwdep_driver get_filter failed\n");
            goto err;
        }
	...
	register_device_proc("Sensor_msensor", YAMAHA530_DEV_NAME, "YAMAHA");
	#endif//VENDOR_EDIT
	MSE_ERR("yamaha530 i2c probe ok!\n");
    return 0;
...
static int yamaha530_i2c_remove(struct i2c_client *client)
{
	...
	if(data != NULL)
	{
	    ...
	    {
		   MSE_ERR("yamaha530_delete_attr fail: %d\n", err);
	    }
