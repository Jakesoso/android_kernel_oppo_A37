...
static int mic_voltage1_comp = 0; /* only for MIC mode = 6 */
int cur_key = 0;
#if !defined(CONFIG_MTK_LEGACY)
...
/*yongzhi.zhang@Multimedia, 2016/03/22, do not create delayed work for hs detection for 15113 project*/
/*xiaojun.lv@Multimedia, 2015/07/08, Add for accdet*/
struct delayed_work hp_detect_work;
/*xiaojun.lv@Multimedia, 2015/07/08, Add for accdet end*/
/*yongzhi.zhang@Multimedia, 2016/03/22, do not create delayed work for hs detection for 15113 project end*/
#endif
...
static int Accdet_PMIC_IMM_GetOneChannelValue(int deCount)
{
	...
	ACCDET_DEBUG("ACCDET accdet_auxadc_offset: %d mv, MIC_Voltage1 = %d mv, mic mode offset: %d!\n\r", accdet_auxadc_offset, vol_val, mic_voltage1_comp);
	return vol_val;
}
...
static inline void headset_plug_out(void)
{
	...
	if (cur_key != 0) {
	/*xiang.fei@Multimedia, 2016/03/17, Delete for headset*/
	#if !defined(OPPO_MTK_MT6750_PROJECT) && !defined(OPPO_MTK_16021_PROJECT) && !defined(OPPO_MTK_15131_PROJECT)
		send_key_event(cur_key, 0);
	#endif
	/*xiang.fei@Multimedia, 2016/03/17, Delete for headset end*/
...
#if defined(OPPO_MTK_MT6750_PROJECT) || defined(OPPO_MTK_15131_PROJECT) || defined(OPPO_MTK_16021_PROJECT)
/*yongzhi.zhang@Multimedia, 2016/03/22, do not create delayed work for hs detection for 15113 project*/
	/*xiaojun.lv@Multimedia, 2015/07/08, Modify for accdet*/
        if(cur_eint_state == EINT_PIN_PLUG_IN)
...
static void multi_key_detection(int current_status)
{
...
#endif
    /*xiang.fei@Multimedia, 2016/03/17, Modify for headset*/
    #if defined(OPPO_MTK_MT6750_PROJECT) || defined(OPPO_MTK_16021_PROJECT) || defined(OPPO_MTK_15131_PROJECT)
        if(cur_eint_state == EINT_PIN_PLUG_IN)
        {   
	...
	#endif
	/*xiang.fei@Multimedia, 2016/03/17, Modify for headset end*/
	} else {
...
#if !defined(OPPO_MTK_MT6750_PROJECT) && !defined(OPPO_MTK_15131_PROJECT) && !defined(OPPO_MTK_16021_PROJECT)
/* yongzhi.zhang@Multimedia, 2015/12/09, modify for modifying the reference voltage of internal voltage comparator in MT6351 */
    pmic_pwrap_write(ACCDET_ADC_REG, pmic_pwrap_read(ACCDET_ADC_REG) | 0x0400);
/* yongzhi.zhang@Multimedia, 2015/12/09, modify for modifying the reference voltage of internal voltage comparator in MT6351 end*/
#endif
