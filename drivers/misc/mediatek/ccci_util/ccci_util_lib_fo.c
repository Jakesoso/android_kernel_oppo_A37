...
static void parse_md_info_tag_val(unsigned int *raw_ptr)
{
	...
	if (active_id == MD_SYS1) {
		//#ifdef VENDOR_EDIT
		//Yilong.Wei@Network.RF, 2016/03/04, Add for compatible RF test tools enter Meta
		/*Remap any -> 00 */
		if (0 != md_info_tag_array[0]) {
			md_info_tag_array[0] = 0;
			CCCI_UTIL_INF_MSG("Remap to 00");
		}
		//#endif
