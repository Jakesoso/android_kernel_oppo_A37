...
void typec_notifier_call_chain()
{
	int type;
	int event;
...
static int typec_platform_hanlder_work(struct work_struct *work)
{
	struct typec_hba *hba = container_of(work, struct typec_hba, platform_hanlder_work);
...
static uint8_t enable_boost = 0;
static uint8_t drive_vbus_action = 0;
static int typec_drive_vbus_work(struct work_struct *work)
{
	struct typec_hba *hba;
 	hba = container_of(work, struct typec_hba, drive_vbus_work);
...
static void typec_set_default_param(struct typec_hba *hba)
{
	...
	typec_writew(hba, (SRC_VRD_DEFAULT_DAC_VAL<<REG_TYPE_C_CC_SRC_VRD_DEFAULT_DAC_VAL_OFST | \
		SRC_VOPEN_DEFAULT_DAC_VAL<<REG_TYPE_C_CC_SRC_VOPEN_DEFAULT_DAC_VAL_OFST), TYPE_C_CC_SRC_DEFAULT_DAC_VAL);
...
int typec_force_term(struct typec_hba *hba, enum enum_typec_term cc1_term, enum enum_typec_term cc2_term)
{
	uint32_t val;
...
static int typec_wait_vbus_on_try_wait_snk(struct work_struct *work)
{
...
static int typec_wait_vbus_on_attach_wait_snk(struct work_struct *work)
{
...
static int typec_wait_vbus_off_attached_snk(struct work_struct *work)
{
...
static int typec_wait_vbus_off_then_drive_attached_src(struct work_struct *work)
{
...
static void typec_intr(struct typec_hba *hba, uint16_t cc_is0, uint16_t cc_is2)
{
	...
	#if ENABLE_ACC
	if (cc_is0 & ((TYPE_C_CC_ENT_ATTACH_SNK_INTR | TYPE_C_CC_ENT_ATTACH_SRC_INTR) | (TYPE_C_CC_ENT_AUDIO_ACC_INTR | TYPE_C_CC_ENT_DBG_ACC_INTR)))
	#else
...
static int __init typec_module_init(void)
{
	...
	if (err = typec_cdev_module_init())
		return err;

	if (err = typec_pltfrm_init())
		goto err_handle;
