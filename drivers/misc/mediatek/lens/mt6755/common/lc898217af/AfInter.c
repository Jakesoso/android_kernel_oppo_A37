...
void RamWriteA(unsigned short addr, unsigned short data)
{
	...
	LC898217_WriteRegI2C(puSendCmd, sizeof(puSendCmd), DeviceAddr);
...
void RamReadA(unsigned short addr, unsigned short *data)
{
	...
	LC898217_ReadRegI2C(puSendCmd, sizeof(puSendCmd), buf, 2, DeviceAddr);
...
void RegReadA(unsigned short addr, unsigned char *data)
{
	...
	LC898217_ReadRegI2C(puSendCmd, sizeof(puSendCmd), data, 1, DeviceAddr);
}
