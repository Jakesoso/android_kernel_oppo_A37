...
typedef enum
{
	...
	e_FLASH_DRIVER_LM3643,  //add by liubin at 20150508
} eDrvierNameId;
...
#define FLASH_IOC_SET_ONOFF_TORCH_A      _IOR(FLASHLIGHT_MAGIC, 116, int)///guanjd add for torch
#define FLASH_IOC_SET_ONOFF_TORCH_D65      _IOR(FLASHLIGHT_MAGIC, 117, int)///guanjd add for torch
#define FLASH_IOC_SET_ONOFF_TORCH_DUAL      _IOR(FLASHLIGHT_MAGIC, 118, int)///guanjd add for torch
#ifdef VENDOR_EDIT
///hufeng add for check flash alive
#define FLASH_IOC_CHECK_ALIVE           _IOR(FLASHLIGHT_MAGIC, 119, int)
#endif
