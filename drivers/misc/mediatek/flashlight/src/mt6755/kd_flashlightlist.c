...
void checkLedAlive()
{
    int partId = g_strobePartId[0][0];
    int partIndex = getPartIndex(partId);
    int retval = -1;
    kdVIOPowerOn(1, SENSOR_DRVNAME_S5K3L8_MIPI_RAW, true, "kd_camera_hw");
    globalInit();
    partId = strobe_getPartId(1, 1);
    g_strobePartId[0][0] = partId;
    partIndex = getPartIndex(partId);
    if(partIndex < 0)
        partIndex = 0;
    setFlashDrv(1, 1);
    retval = g_pFlashInitFunc[0][0][partIndex]->flashlight_ioctl(FLASH_IOC_SET_DUTY, 0);
    if (retval>=0&&retval<0xff)
        flashlight_state = 7;
    else
        flashlight_state = 8;
    kdVIOPowerOn(1, SENSOR_DRVNAME_S5K3L8_MIPI_RAW, false, "kd_camera_hw");
                                                                     ...
                                                                     loff_t *data)
{
    ...
    if (copy_from_user(regBuf, buffer, u4CopyBufSize))
        return -EFAULT;
    logI("write flash state:%d flashlight_state:%d\n",regBuf[0] - '0',flashlight_state);
...
/*oppo hufeng add end*/
    ...
    {
        logI("flash state is same, do not need set flash \n");
        return count;
    }

    if (main_sensor_state || sub_sensor_state)
    {
        if (regBuf[0] == '5' && flashlight_state == 1)
        {
            flashlight_state = regBuf[0] - '0';
            if(partIndex < 0)
                partIndex = 0;
            if(g_pFlashInitFunc[0][0][partIndex] != NULL)
                g_pFlashInitFunc[0][0][partIndex]->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
			kdVIOPowerOn(1, SENSOR_DRVNAME_S5K3L8_MIPI_RAW, false, "kd_camera_hw");
        }
        logI("sensor is poweron ,not to set flash\n");
        return count;
    }
    if (flashlight_state == 5 && regBuf[0] == '0')
    {
        logI("camera is open ,not to set flash\n");
        return count;
    }
    ...
    if(regBuf[0] == '0')
    {
        ...
        if(regBuf[0] == '0')
        {
            g_pFlashInitFunc[0][0][partIndex]->flashlight_release(0);
            g_pFlashInitFunc[0][0][partIndex] = 0;//jindian.guan add 20150421
        }
        kdVIOPowerOn(1, SENSOR_DRVNAME_S5K3L8_MIPI_RAW, false, "kd_camera_hw");
    }
    else if(regBuf[0] == '1')
    {
        flashlight_state = 1;
        g_pFlashInitFunc[0][0][partIndex]->flashlight_ioctl(FLASH_IOC_SET_DUTY, 0);
        g_pFlashInitFunc[0][0][partIndex]->flashlight_ioctl(FLASH_IOC_SET_STEP, 0);
        g_pFlashInitFunc[0][0][partIndex]->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 0);
        g_pFlashInitFunc[0][0][partIndex]->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 1);
    }
...
static int flash_proc_init(void)
{
	...
	{
		ret = -ENOMEM;
	  	logI("Error! Couldn't create qcom_flash proc entry\n");
	}
	return ret;
}
