...
#ifdef VENDOR_EDIT
/*Liubin@Camera, 2014/10/09, add for flashlight driver*/
#include <linux/proc_fs.h>       //proc file use
#include "kd_camera_feature.h"
#include "kd_imgsensor.h"
//LiuBin@Camera, 2015/04/11, Add for close flashlight in factory mode
...
//====================================================================================
#ifdef VENDOR_EDIT
/*zhangkw@Camera, 2015/04/23, add for flashlight driver*/
#define FL_HW_NAME 			"kd_flashlight_oppo"

static struct i2c_client * g_pstFL_I2Cclient = NULL;

static int FL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) 
{             
    int i4RetValue = 0;
    logE("[FL_i2c_probe] Attach I2C \n");
 
    g_pstFL_I2Cclient = client;
    logE("[FL_i2c_probe]  g_pstFL_I2Cclient(%p), client(%p)", g_pstFL_I2Cclient, client);	
    return i4RetValue;                                                                                       
} 
static int FL_i2c_detect(struct i2c_client *client,  struct i2c_board_info *info) {         
    strcpy(info->type, FL_HW_NAME);       
     logE("[FL_i2c_detect] Attach I2C \n");	
    return 0;                                                                                       
}                                                                                                  
static int FL_i2c_remove(struct i2c_client *client) {
    return 0;
}
#ifdef CONFIG_OF
    static const struct of_device_id Flash_HW_i2c_of_ids[] = {
	{ .compatible = "mediatek,CAMERA_HW_i2C", },
	{}
    };
#endif

static const struct i2c_device_id FL_i2c_id[] = {{FL_HW_NAME,0},{}};   
static struct i2c_board_info  fl_i2c_info __initdata ={ I2C_BOARD_INFO(FL_HW_NAME, 0x63)};

struct i2c_driver FL_i2c_driver = {                       
    .probe = FL_i2c_probe,                                   
    .remove = FL_i2c_remove,    
    .detect = FL_i2c_detect,
    .id_table = FL_i2c_id,                             
    .driver = {
    .name	= FL_HW_NAME,
	.owner = THIS_MODULE,		
#ifdef CONFIG_OF
	.of_match_table = Flash_HW_i2c_of_ids,
#endif
...
int iWriteStrobeReg(char reg, char data)
{
    int  i4RetValue = 0;
    int retry = 3; 
    char pSendData[2] = {reg, data};

    do
	{
        i4RetValue = i2c_master_send(g_pstFL_I2Cclient, pSendData, 2);
        if (i4RetValue != 2) 
	 	{
            logE("[FLASHLIGHT] I2C send failed!!, Addr = 0x%x, Data = 0x%x \n", pSendData[0], pSendData[1] );
        }
        else 
		{
            break; 
...
int iReadStrobeReg(char reg, char * data)
{
    int  i4RetValue = 0;
    i4RetValue = i2c_master_send(g_pstFL_I2Cclient, &reg, 1);
    if (i4RetValue != 1) 
    {
        logE("[FLASHLIGHT] I2C send failed!!, Addr = 0x%x\n", reg);
        return -1; 
    }

    i4RetValue = i2c_master_recv(g_pstFL_I2Cclient, data, 1);
    if (i4RetValue != 1) 
    {
		logE("[FLASHLIGHT] I2C read failed!! \n");
		return -1; 
    }
    return 0;
}
...
static long flashlight_ioctl_core(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	unsigned long copyRet;
    copyRet = copy_from_user(&kdArg , (void *)arg , sizeof(kdStrobeDrvArg));
	logI("flashlight_ioctl cmd=0x%x(nr=%d), senorDev=0x%x ledId=0x%x arg=0x%lx, g_pstFL_I2Cclient(%p)",
		cmd, _IOC_NR(cmd), kdArg.sensorDev, kdArg.strobeId ,(unsigned long)kdArg.arg,g_pstFL_I2Cclient);
...
/*****************************************************************************/
#ifdef VENDOR_EDIT
/*Liubin@Camera, 2014/10/09, add proc to support debug of factory-mode*/
...
/**************************************************************************
  * FL_HW_Debug() 
  * Used for DEBUG by proc file 
  **************************************************************************/
extern int main_sensor_state;
extern int sub_sensor_state;
                                                                     ...
                                                                     loff_t *data)
{
    ...
    if (copy_from_user(regBuf, buffer, u4CopyBufSize))
        return -EFAULT;
    logE("write flash state:%d flashlight_state:%d\n",regBuf[0] - '0',flashlight_state);
    ...
    {
        logE("flash state is same, do not need set flash \n");
        return count;
    }

    if (main_sensor_state)
    {
        logE("sensor is poweron ,not to set flash\n");
        return count;
    }
    if (flashlight_state == 5 && regBuf[0] == '0')
    {
        logE("camera is open ,not to set flash\n");
        return count;
    }
    ...
    if(regBuf[0] == '1'||regBuf[0] == '2'||regBuf[0] == '3')
    {
        if(sub_sensor_state == 0){
            logE("sub camera is open ,not to set power\n");
            kdVIOPowerOn(1, SENSOR_DRVNAME_IMX298_MIPI_RAW, true, "kd_camera_hw");
            }
        globalInit();
        setFlashDrv(1, 1);
    }

    if (g_pFlashInitFunc[0][0][0] == NULL)
    {
        logE("g_pFlashInitFunc is NULL \n");
        return count;
    }
	
    if(regBuf[0] == '0' || regBuf[0] == '5')
    {
        flashlight_state = regBuf[0] - '0';
        if(sub_sensor_state == 0)
            kdVIOPowerOn(1, SENSOR_DRVNAME_IMX298_MIPI_RAW, true, "kd_camera_hw");
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_ONOFF_TORCH_DUAL, 0);
        if(regBuf[0] == '0')
        {
        g_pFlashInitFunc[0][0][0]->flashlight_release(0);
        g_pFlashInitFunc[0][0][0] = 0;//jindian.guan add 20150421
        }
        if(sub_sensor_state == 0)
            kdVIOPowerOn(1, SENSOR_DRVNAME_IMX298_MIPI_RAW, false, "kd_camera_hw");
    }
    else if(regBuf[0] == '1')
    {
        flashlight_state = 1;
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_DUTY, 0);
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_STEP, 0);
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 0);
        //g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_ONOFF_TORCH_D65, 1);
    }
    else if(regBuf[0] == '2')
    {
        flashlight_state = 1;
        ...
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 0);
        //g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_ONOFF_TORCH_A, 1);
    }
    else if(regBuf[0] == '3')
    {
        flashlight_state = 1;
        ...
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 0);
        //g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
        g_pFlashInitFunc[0][0][0]->flashlight_ioctl(FLASH_IOC_SET_ONOFF_TORCH_DUAL, 1);
    }
...
static int flash_proc_init(void)
{
	...
	{
		ret = -ENOMEM;
	  	logE("Error! Couldn't create qcom_flash proc entry\n");
	}
	return ret;
}
...
static int flashlight_probe(struct platform_device *dev)
{
...
#ifdef VENDOR_EDIT	
    /*zhangkw@Camera, 2015/04/23, add for flash i2c driver*/
    if(i2c_add_driver(&FL_i2c_driver) != 0){
        logE("i2c_add_driver error \n");
    }
#endif /*VENDOR_EDIT*/
	...
	#ifdef VENDOR_EDIT
    /*Liubin@Camera, 2014/10/09, add proc for flashlight debug*/
	flash_proc_init();
    #endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT  
    /*zhangkw@Camera, 2015/04/23, add for delete flash i2c driver*/
    i2c_del_driver(&FL_i2c_driver);
#endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT
    /*zhangkw@Camera, 2015/04/23, add for register i2c board info*/
    ret = i2c_register_board_info(2, &fl_i2c_info, 1);
	if (ret) {
        logE("[flashlight_probe] i2c_register_board_info fail ~");
	}
#endif /*VENDOR_EDIT*/
