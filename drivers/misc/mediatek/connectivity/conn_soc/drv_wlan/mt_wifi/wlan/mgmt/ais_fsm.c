			...
			if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_DISCONNECTED) {
				...
				/* 4 <2.b> If we don't have the matched one */
					...
					if (prAisFsmInfo->rJoinReqTime != 0 &&
						CHECK_FOR_TIMEOUT(kalGetTimeTick(), prAisFsmInfo->rJoinReqTime,
...
VOID configDelBaToFw(P_ADAPTER_T prAdapter, BOOLEAN fgEnable)
{
	  ...
	  if (pcmdV1Header == NULL) {
		...
		P_CMD_FORMAT_V1_T pr_cmd_v1 = (P_CMD_FORMAT_V1_T) pcmdV1Header->buffer;

		pr_cmd_v1->itemType=ITEM_TYPE_DEC;
		pr_cmd_v1->itemStringLength=strlen(itemString);
		strncpy(pr_cmd_v1->itemString, itemString,  pr_cmd_v1->itemStringLength);
		pr_cmd_v1->itemValueLength=1;
		if (fgEnable)
				pr_cmd_v1->itemValue[0]=1;
		else
				pr_cmd_v1->itemValue[0]=0;
