...
BOOLEAN rsnCheckSecurityModeChanged(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo, P_BSS_DESC_T prBssDesc)
{
	...
	BOOLEAN fgModeChange = FALSE;	
	static UINT_8 ucModeChangeCnt;
	...
	case AUTH_MODE_WPA_NONE:
		if (prBssDesc->fgIEWPA) {
			if (rsnCheckWpaRsnInfo(prBssInfo, &prBssDesc->rWPAInfo))
				fgModeChange = TRUE;
			break;
		}
	...
	case AUTH_MODE_WPA2_PSK:
		if (prBssDesc->fgIERSN) {
			if (rsnCheckWpaRsnInfo(prBssInfo, &prBssDesc->rRSNInfo))
				fgModeChange = TRUE;
			break;
		}
