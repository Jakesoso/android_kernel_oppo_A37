...
static int _check_dvfs_result(int vcore, int ddr)
{
	...
	if (dvs_en == true) {
		...
		if (vcore_val != vcore) {
			...
			for (i=0; i<5; i++)
			{
				ret = pmic_read_interface_nolock(MT6353_DEW_READ_TEST, &reg_val, 0xffff, 0);
				spm_notice("[PMIC](%d)dew_read_test=0x%x ret=0x%x\n", i, reg_val, ret);
				ret = pmic_read_interface_nolock(MT6353_BUCK_VCORE_VOL_CON0, &reg_val, 0xffff, 0);
				spm_notice("[PMIC](%d)vosel_result=0x%x ret=0x%x\n", i, reg_val, ret);
				ret = pmic_read_interface_nolock(MT6353_BUCK_VCORE_VOL_CON2, &reg_val, 0xffff, 0);
				spm_notice("[PMIC](%d)vosel_on=0x%x ret=0x%x\n", i, reg_val, ret);
				udelay(1);
			}		
#endif
			BUG();
...
#if defined(CONFIG_MTK_PMIC_CHIP_MT6353)
	ret = pmic_read_interface_nolock(MT6353_DEW_READ_TEST, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]dew_read_test=0x%x ret=0x%x\n", reg_val, ret);
	ret = pmic_read_interface_nolock(MT6353_HWCID, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]hw_cid=0x%x ret=0x%x\n", reg_val, ret);
	ret = pmic_read_interface_nolock(MT6353_SWCID, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]sw_cid=0x%x ret=0x%x\n", reg_val, ret);
	ret = pmic_read_interface_nolock(MT6353_WDTDBG_CON1, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]wdtdbg_con1=0x%x ret=0x%x\n", reg_val, ret);
	ret = pmic_read_interface_nolock(MT6353_BUCK_VCORE_HWM_CON0, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]vcore vosel_ctrl=0x%x ret=0x%x\n", reg_val, ret);
	ret = pmic_read_interface_nolock(MT6353_BUCK_VCORE_VOL_CON1, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]vcore vosel=0x%x ret=0x%x\n", reg_val, ret);
	ret = pmic_read_interface_nolock(MT6353_BUCK_VCORE_VOL_CON2, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]vcore vosel_on=0x%x ret=0x%x\n", reg_val, ret);
	ret = pmic_read_interface_nolock(MT6353_BUCK_VCORE_VOL_CON0, &reg_val, 0xffff, 0);
	spm_notice("[PMIC]vcore vosel result=0x%x ret=0x%x\n", reg_val, ret);
#else
