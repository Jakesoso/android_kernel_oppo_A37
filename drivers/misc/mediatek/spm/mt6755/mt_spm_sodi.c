...
m4u_exit:
	...
	if (!node) {
		sodi_err("error: cannot find node " VCORE_DVFS_NODE);
		goto vcore_dvfs_exit;
	}
	if (of_address_to_resource(node, 0, &r)) {
		sodi_err("error: cannot get phys addr" VCORE_DVFS_NODE);
		goto vcore_dvfs_exit;
	}
	...
	if (!vcore_dvfs_base) {
		sodi_err("error: cannot iomap " VCORE_DVFS_NODE);
		goto vcore_dvfs_exit;
	}

	sodi_debug("vcore_dvfs_base = 0x%u\n", (unsigned int)vcore_dvfs_base);
