...
#ifdef VENDOR_EDIT
/*oppo hufeng add for read otp at phone boot*/
#define MAX_EEPROM_SIZE 4020
static u8 eeprom_data[MAX_EEPROM_SIZE];
static u32 EepromDataAddr[MaxMapNum]={SHAREDDATAADDR, BLANKDATAADDR, OISDATAADDR, QCOMDATAADDR, MTKLSCDATAADDR, MTKPDAFDATAADDR};
static u32 EepromDataSize[MaxMapNum]={SHAREDDATALEN, BLANKDATALEN, OISDATALEN, QCOMDATALEN, MTKLSCDATALEN, MTKPDAFDATALEN};
static u32 EepromDataValid[MaxMapNum]={SHAREDDATAFALG, BLANKDATAFLAG, OISDATAFLAG, QCOMDATAFLAG, MTKLSCDATAFLAG, MTKPDAFDATAFLAG};
static u32 EepromDataIndex[MaxMapNum];
/*************************************************************************
* FUNCTION
*	convert_addr_to_index
*
* DESCRIPTION
*	This function convert address to index of the array
*
* PARAMETERS
*	*sensorID : return the index of the array
*
* RETURNS
*	None
*
...
*
*************************************************************************/
static int convert_addr_to_index(u32 addr)
{
	u32 array_index = 0;
	u32 sum_index=0;
	int i,j;
	...
	{
		if (EepromDataAddr[i]==addr)
		{
			for (j=0;j<i;j++)
				sum_index+=EepromDataSize[j]*EepromDataValid[j];
			array_index = sum_index;
			CAM_CALDB("the addr is %d, after converted, the array_index is %d\n", addr, array_index);
			return array_index;
		}
		else if (EepromDataAddr[i]>addr)
		{
			for (j=0;j<i;j++)
				sum_index+=EepromDataSize[j]*EepromDataValid[j];
			array_index = sum_index-(EepromDataAddr[i]-addr);
			CAM_CALDB("the addr is %d, after converted, the array_index is %d\n", addr, array_index);
			return array_index;
		}
		if (i==(MaxMapNum-1))
		{
			for (j=0;j<i;j++)
				sum_index+=EepromDataSize[j]*EepromDataValid[j];
			array_index = (sum_index-1)-(EEPROMENDADDR-addr);
			CAM_CALDB("the addr is %d, after converted, the array_index is %d\n", addr, array_index);
			return array_index;
...
/*************************************************************************
* FUNCTION
*	convert_index_to_addr
*
* DESCRIPTION
*	This function convert the index of array to address
*
* PARAMETERS
*	*sensorID : return the address
*
* RETURNS
*	None
*
...
*
*************************************************************************/
static int convert_index_to_addr(u32 index)
{
	u32 addr = 0;
	u32 sum_index=0;
	u32 sum_addr=0;
	...
	for (i=0;i<MaxMapNum;i++)
	{
		sum_index+=EepromDataSize[i]*EepromDataValid[i];
		if (index<sum_index)
		{
			if (i<(MaxMapNum-1))
				addr=EepromDataAddr[i+1]-(sum_index-index);
			else
				addr=(EEPROMENDADDR+1)-(sum_index-index);
			CAM_CALDB("the index is %d, after converted, the addr is 0x%x\n", index, addr);
			return addr;
		}
	}
	return 0;
...
*
* DESCRIPTION
*	This function read all valid eeprom registers and store them to a static array
*
* PARAMETERS
*	NONE
*
...
*
*************************************************************************/
void read_otp_static_array()
{
	unsigned short curAddr = 0x0000;
	u8 *buff = eeprom_data;
	u32 size_to_read = 0;
	u32 index = 0;
	int ret = 0;
	...
	for (i=0; i<MaxMapNum;i++)
	{
		if (EepromDataValid[i])
			size_to_read += EepromDataSize[i];
	}
	for (i=0;i<size_to_read;i++)
	{
		curAddr = convert_index_to_addr(index);
		byteread_cmos_sensor(CUR_EEPROM_DEVICE_ID, curAddr, buff);
		CAM_CALDB("after byteread_cmos_sensor curAddr =%x index=%d buffData=%x\n", curAddr,
		index, *buff);
		buff += 1;
		index ++;
	}
...
static int read_regs_from_array(u32 addr, u8 *data, u32 size)
{
	unsigned short curAddr = (unsigned short)addr;
	u8 *buff = data;
	u32 size_to_read = size;
	u32 index = 0;
	index = convert_addr_to_index(curAddr);
	memcpy(buff, &eeprom_data[index], size_to_read);
	CAM_CALDB("selective_read_region addr =%x size %d\n", addr, size);
	return size_to_read;
}
#endif
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		/**pu1Params = 0;*/
#ifdef VENDOR_EDIT
/*oppo hufeng add for otp*/
		i4RetValue = read_regs_from_array(ptempbuf->u4Offset, pu1Params, ptempbuf->u4Length);
#else
