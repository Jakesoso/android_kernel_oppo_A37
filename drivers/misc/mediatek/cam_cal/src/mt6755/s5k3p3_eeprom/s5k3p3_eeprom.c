...
*
********************************************************************************/
#define CAM_CAL_DRVNAME_3P3   "CAM_CAL_DRV2"
#define CAM_CAL_I2C_GROUP_ID 0
/*******************************************************************************
*
********************************************************************************/
static struct i2c_board_info kd_cam_cal_dev __initdata = { I2C_BOARD_INFO(CAM_CAL_DRVNAME_3P3, 0xA2 >> 1)};
...
/*oppo hufeng 20160107 add to solve eeprom and sensor conflict*/
	...
	if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
		CAM_CALERR("[CAM_CAL] fail s5k3p3_byteread_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
	...
	*data,page,offset);*/
	CAM_CALDB("[CAM_CAL] s5k3p3_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
	return true;
...
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		/*if(selective_read_byte(addr,(u8*)buff,S5K3P3_DEVICE_ID)){*/
...
static long s5k3p3_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret;
	COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
	stCAM_CAL_INFO_STRUCT __user *data;
	int err;
	CAM_CALDB("[CAMERA SENSOR] s5k3p3_Ioctl_Compat,%p %p %x ioc size %d\n", filp->f_op ,
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		/**pu1Params = 0;*/
		i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, S5K3P3_DEVICE_ID, ptempbuf->u4Length);
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv2);
		if (ktv2.tv_sec > ktv1.tv_sec)
			TimeIntervalUS = ktv1.tv_usec + 1000000 - ktv2.tv_usec;
...
static const struct file_operations g_stCAM_CAL_fops = {
	...
	/*.ioctl = CAM_CAL_Ioctl*/
#ifdef CONFIG_COMPAT
	.compat_ioctl = s5k3p3_Ioctl_Compat,
#endif
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 1, 1, CAM_CAL_DRVNAME_3P3)) {
		CAM_CALDB("[CAM_CAL] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno , 1 , CAM_CAL_DRVNAME_3P3)) {
	...
	CAM_CAL_device = device_create(CAM_CAL_class, NULL, g_CAM_CALdevno, NULL, CAM_CAL_DRVNAME_3P3);
...
static const struct i2c_device_id CAM_CAL_i2c_id[] = {{CAM_CAL_DRVNAME_3P3, 0}, {} };
...
static struct i2c_driver CAM_CAL_i2c_driver = {
	...
	.driver.name = CAM_CAL_DRVNAME_3P3,
...
static int CAM_CAL_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info)
{
	strcpy(info->type, CAM_CAL_DRVNAME_3P3);
	return 0;
}
#endif
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr = S5K3P3_DEVICE_ID >> 1;
...
MODULE_DESCRIPTION("S5K3P3 CAM_CAL driver");
