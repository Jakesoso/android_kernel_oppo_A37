...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
    ...
    else if(ui4_offset == 0x7042){//read ov8865 af infi
        *pinputdata = OV8865R_AF[0];
        *(pinputdata+1) = OV8865R_AF[1];
    }
    else if(ui4_offset == 0x7041){//read ov8865 af macro
        *pinputdata = OV8865R_AF[2];
        *(pinputdata+1) = OV8865R_AF[3];
    }
    else if(ui4_offset == 0x7011){//read ov8865 id
        *pinputdata = OV8865R_AF[4];
        CAM_CALDB( "[[CAM_CAL]]vendor id=%d\n",*pinputdata);
    }
    else {
        IMX179_ReadOtp(ui4_offset,pinputdata, ui4_length);  
