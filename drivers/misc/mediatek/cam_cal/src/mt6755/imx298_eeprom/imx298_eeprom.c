...
static bool byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data)
{
	...
	if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
		CAM_CALERR("[CAM_CAL] fail imx298_byteread_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
		return false;
	}
...
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		/*if(selective_read_byte(addr,(u8*)buff,imx298_DEVICE_ID)){*/
...
static void IMX298_ReadOtp(kal_uint16 address,unsigned char *iBuffer,unsigned int buffersize)
{
    kal_uint16 i = 0;
    u8 readbuff ;
    int ret ;
    u16 i2c_id = IMX298_DEVICE_ID;
...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
    int  i4RetValue = 0;
     ...
     /* read otp */
    IMX298_ReadOtp(ui4_offset,pinputdata, ui4_length);
    ...
    CAM_CALDB("[IMX298EEPORM] iReadData done\n" );
   return ui4_length;
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr = IMX298_DEVICE_ID >> 1;
	spin_unlock(&g_CAM_CALLock); /* for SMP*/
...
MODULE_DESCRIPTION("IMX298 CAM_CAL driver");
