	...
	for (i = 0; i < mtk_ldos_size; i++) {
		if (mtk_ldos[i].isUsedable==1)
		{
			if (mtk_ldos[i].vol_reg != 0) {
				...
				/* if (mtk_regulator_get_voltage_sel(mtk_ldos[i].rdev) != mtk_ldos[i].vosel.cur_sel) {*/
...
bool hwPowerOn(MT65XX_POWER powerId, int powerVolt, char *mode_name)
{
	...
	if (ret2 != 0) {
		pr_err("hwPowerOn:%s can't set the same volt %d again.", mtk_ldos[powerId].desc.name, powerVolt);
		pr_err("Or please check:%s volt %d is correct.", mtk_ldos[powerId].desc.name, powerVolt);
	}
