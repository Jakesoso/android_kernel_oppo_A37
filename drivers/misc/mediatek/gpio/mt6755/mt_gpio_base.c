...
int vooc_uart_mt_set_gpio_out_base(unsigned long pin, unsigned long output)
{
    ...
    if (output == GPIO_OUT_ZERO)
        VOOC_UART_GPIO_SET_BITS((1L << bit), &reg->dout[pos].rst);
    else
        VOOC_UART_GPIO_SET_BITS((1L << bit), &reg->dout[pos].set);
    return RSUCCESS;
}
#endif
...
int mt_set_gpio_mode_base(unsigned long pin, unsigned long mode)
{
	...
	unsigned long base = (unsigned long) GPIO_BASE + MODE_addr[pin].addr;
	GPIO_WR32(base, data);
#endif
