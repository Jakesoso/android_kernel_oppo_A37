...
static int get_fifo_data(struct batch_context *obj)
{
		...
		&& (obj->dev_list.data_dev[ID_SENSOR_MAX_HANDLE].get_fifo_status)!=NULL)
	{
		...
		else if(fifo_len>=0)
		{
			#ifdef CONFIG_PM_WAKELOCKS
			#ifndef VENDOR_EDIT
			//zhihong.lu,2016/4/27,modify to fix incompatible pointer type err
			__pm_stay_awake(&(batch_context_obj->read_data_wake_lock));
			#else /*VENDOR_EDIT*/
			__pm_stay_awake(&(batch_context_obj->read_data_wake_lock.ws));
			#endif /*VENDOR_EDIT*/
			#else
    ...
    switch(cmd)
    {
        case BATCH_IO_GET_SENSORS_DATA:
			...
			if (batch_context_obj->numOfDataLeft == 0) {
				...
				#else /*VENDOR_EDIT*/
				__pm_relax(&(batch_context_obj->read_data_wake_lock.ws));
				#endif /*VENDOR_EDIT*/
#else
...
static int batch_probe(struct platform_device *pdev) 
{
	...
	#else /*VENDOR_EDIT*/
	wakeup_source_init(&(batch_context_obj->read_data_wake_lock.ws),"read_data_wake_lock");
	#endif /*VENDOR_EDIT*/
	#else
