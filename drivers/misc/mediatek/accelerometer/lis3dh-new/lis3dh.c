...
extern void register_vibrator_notify(void (*notify)(bool));
...
struct lis3dh_i2c_data {
    ...
    #ifdef VENDOR_EDIT
    /*Fei.Mo@EXP.BSP.Sensor, 2017/06/13, Add for solve vibrator noise */
...
static int LIS3DH_ReadData(struct i2c_client *client, s16 data[LIS3DH_AXES_NUM])
{
	...
	//GSE_LOG("[%08X %08X %08X %08x %08x %08x]\n",buf[0],buf[1],buf[2],buf[3],buf[4],buf[5]);
		...
		#ifdef VENDOR_EDIT
		/*Fei.Mo@EXP.BSP.Sensor, 2017/06/26, Add for solve vibrator noise */
		if (priv->vibrator_enabled) {
			atomic_set(&priv->filter, 1);
			atomic_set(&priv->fir_en, 1);
			atomic_set(&priv->firlen, 12);//6->12
		} else if ((!priv->vibrator_enabled)){
			atomic_set(&priv->filter, 0);
			atomic_set(&priv->fir_en, 0);
			atomic_set(&priv->firlen, 0);
		}
		#endif /* VENDOR_EDIT */
...
/*Fei.Mo@EXP.BSP.Sensor, 2017/06/13, Add for solve vibrator noise */
#if defined(CONFIG_LIS3DH_LOWPASS)
static void vibrator_notify_handle(bool enable)
{
	if (!obj_i2c_data) {
		GSE_ERR("obj_i2c_data NULL\n");
		return;
	}
	obj_i2c_data->vibrator_enabled = enable;
}
#endif
#endif /* VENDOR_EDIT */
/*----------------------------------------------------------------------------*/
...
static int lis3dh_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#ifdef VENDOR_EDIT
	/*Fei.Mo@EXP.BSP.Sensor, 2017/06/13, Add for solve vibrator noise */
	register_vibrator_notify(vibrator_notify_handle);
	#endif /* VENDOR_EDIT */
