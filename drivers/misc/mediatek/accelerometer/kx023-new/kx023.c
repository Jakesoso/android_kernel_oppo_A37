...
static DEFINE_MUTEX(kx023_i2c_mutex);
...
/*----------------------------------------------------------------------------*/
static int KX023_i2c_read_block(struct i2c_client *client, u8 addr, u8 *buf, int len)
{
...
/*----------------------------------------------------------------------------*/
static int KX023_i2c_write_block(struct i2c_client *client, u8 *buf, int len)
{
	int ret;
...
static int KX023_SetDataResolution(struct kx023_i2c_data *obj)
{
	...
	err = KX023_i2c_read_block(obj->client, KX023_REG_DATA_RESOLUTION, databuf, 0x01);
	...
	err = KX023_i2c_write_block(obj->client, databuf, 0x2);
...
static int KX023_ReadData(struct i2c_client *client, s16 data[KX023_AXES_NUM])
{
	...
	err = KX023_i2c_read_block(client, addr, buf, 0x06);
	if (err)
	{
		GSE_ERR("error: %d\n", err);
	}
...
static int KX023_WriteCalibration(struct i2c_client *client, int dat[KX023_AXES_NUM])
{
	...
	data[0] = KX023_REG_OFSX;
	data[1] = obj->offset[KX023_AXIS_X];
	data[2] = obj->offset[KX023_AXIS_Y];
	data[3] = obj->offset[KX023_AXIS_Z];
	err = KX023_i2c_write_block(obj->client, data, KX023_AXES_NUM + 1);
	if (err)
	{
...
#else
	res = KX023_i2c_read_block(client, addr, databuf, 0x01);
#endif
	if(res)
	{
		printk("KX023_CheckDeviceID res=%d!\n ", res);
		goto exit_KX023_CheckDeviceID;
	}
...
static int KX023_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	res = KX023_i2c_write_block(client, databuf, 0x2);
	if(res)
	{
		return KX023_ERR_I2C;
	}
...
static int KX023_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	res = KX023_i2c_read_block(client, KX023_REG_DATA_FORMAT, databuf, 0x01);
	if (res)
	{
		printk("kx023 read Dataformat failt \n");
		return KX023_ERR_I2C;
...
static int KX023_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	res = KX023_i2c_read_block(client, KX023_REG_BW_RATE, databuf, 0x01);
	if (res)
	{
...
static int KX023_InitSelfTest(struct i2c_client *client)
{
	...
	res = KX023_i2c_read_block(client, KX023_REG_CTL_REG2, data, 1);
	if(res != KX023_SUCCESS)
	{
		return res;
	}
...
//enable selftest bit
	data[0] = KX023_REG_CTL_REG2;
	data[1] |= KX023_SELF_TEST;
	res = KX023_i2c_write_block(client, data, 2);
	if(res != KX023_SUCCESS) //0x2C->BW=100Hz
	{
		return res;
	}
//step 1
	res = KX023_i2c_read_block(client, KX023_DCST_RESP, &result, 1);
	if(res != KX023_SUCCESS)
	{
		return res;
	}
...
static ssize_t store_register_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	res = KX023_i2c_write_block(obj->client, databuf, 0x2);
	if(res)
	{
		return KX023_ERR_I2C;
	}
...
static ssize_t show_register_value(struct device_driver *ddri, char *buf)
{
		...
		ret = KX023_i2c_read_block(obj->client, i2c_dev_reg, databuf, 0x01);
		if (ret)
		{
			GSE_ERR("read power ctl register err!\n");
			return KX023_ERR_I2C;
...
static int kx023_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = KX023_CheckDeviceID(new_client); 
	if(err != KX023_SUCCESS)
	{
		GSE_ERR("KX023_CheckDeviceID failed\n");
		goto exit_init_failed;
	}

	if((err = kx023_init_client(new_client, 1)))
	{
		GSE_ERR("kx023_init_client failed\n");
		goto exit_init_failed;
	}
