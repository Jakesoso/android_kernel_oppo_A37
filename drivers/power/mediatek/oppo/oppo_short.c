...
static int short_c_feature_status = SHORT_C_BATT_FEATURE_STATUS__ON;
...
static int __init oppo_short_c_feature_status_init(char *str)
{
	sscanf(str, "%d", &short_c_feature_status);
	chg_err("short_c_feature_status[%d]\n", short_c_feature_status);
	return 0;
}
__setup("short_c_feature_status=", oppo_short_c_feature_status_init);
...
{
	if (chip->short_c_batt.is_feature_on == SHORT_C_BATT_FEATURE_STATUS__OFF)
		return false;
...
{
	if (chip->short_c_batt.cv_satus == true || chip->batt_full == true)
		return true;
	return false;
}
...
static int oppo_short_c_batt_read_file(struct oppo_chg_chip *chip)
{
	...
	for (i = 0; i < ARRAY_SIZE(short_c_batt_items); i++) {
		...
		if (p != 0) {
			...
			if (ret != 0) {
				...
				if (short_c_batt_items[i].value == abs(INVALID_DATA))
					short_c_batt_items[i].value = INVALID_DATA;
				chg_err("%s[%d]\n", short_c_batt_items[i].name, short_c_batt_items[i].value);
				short_debug("%s[%d]\n", short_c_batt_items[i].name, short_c_batt_items[i].value);
			}
	...
	chip->short_c_batt.cv_timer1 = short_c_batt_items[2].value / 5;
	chip->short_c_batt.full_timer2 = short_c_batt_items[3].value / 5;
	chip->short_c_batt.full_timer3 = short_c_batt_items[4].value / 5;
	chip->short_c_batt.full_timer4 = short_c_batt_items[5].value / 5;
	chip->short_c_batt.full_timer5 = short_c_batt_items[6].value / 5;
	...
	chip->short_c_batt.is_feature_on = short_c_batt_items[26].value;
...
#define INPUT_LIMIT_MA_THRESHOLD	1000
...
void oppo_chg_short_c_battery_check(struct oppo_chg_chip *chip)
{
	...
	if (chip->short_c_batt.update_change == 1) {
		chip->short_c_batt.update_change = 0;
	...
	if (chip->temperature <= LITTLE_COLD_TEMP_THRESHOLD || chip->temperature > WARM_TEMP_THRESHOLD) {//0~45
		...
		if (oppo_short_c_batt_get_cv_status(chip) == true) {
			...
			chg_err("exit_check, temperature[%d]\n", chip->temperature);
			short_debug("exit_check, out of temp\n");
			exit_check_status = true;
			return;
		}
		else
	...
	if (oppo_short_c_batt_get_cv_status(chip) == true && chip->short_c_batt.in_idle == false) {
		short_debug("exit_check, idle false\n");
		chg_err("exit_check, idle false\n");
		exit_check_status = true;
		return;
	}
	...
	if (chip->batt_full == true) {
		...
		if (full_over_10ma_count >= chip->short_c_batt.ex2_lower_ibatt_count) {
			...
			chg_err("exit_check, over [%d]mA [%d]times\n",
					chip->short_c_batt.ex2_lower_ibatt_ma, chip->short_c_batt.ex2_lower_ibatt_count);
			short_debug("exit_check, full_over_10ma_count[%d]\n", full_over_10ma_count);
			exit_check_status = true;
			return;
		}
		if (full_over_20ma_count >= chip->short_c_batt.ex2_low_ibatt_count) {
			...
			short_debug("exit_check, full_over_20ma_count[%d]\n", full_over_20ma_count);
			exit_check_status = true;
			return;
		}
		if (full_over_50ma_count >= chip->short_c_batt.ex2_high_ibatt_count) {
			...
			short_debug("exit_check, full_over_50ma_count[%d]\n", full_over_50ma_count);
			exit_check_status = true;
			return;
		}
		...
		if (full_timer5_count >= chip->short_c_batt.full_timer5
			...
			short_debug("full_timer5_count[%d], full_vbatt_2[%d]\n", full_timer5_count, full_vbatt_2);
			if (full_vbatt_2 > 0 && chip->limits.iterm_ma != 0) {
				...
				if (cv_avg_vbatt > chip->short_c_batt.short_c_bat_cv_mv + 1000
					...
					cv_avg_vbatt = chip->short_c_batt.short_c_bat_cv_mv;
		...
		/* "==" means timer5 overflow and start timer2 */
		if (full_timer5_count < chip->short_c_batt.full_timer5 + 2)
			return;
		...
		if (full_timer2_count == chip->short_c_batt.full_timer2)
			oppo_short_c_batt_get_average_vbatt_mv(chip, true);//reset
		if (full_timer2_count > chip->short_c_batt.full_timer2) {//timer2 overflow
			full_timer2_count = chip->short_c_batt.full_timer2 + 1;

			/* start timer3 */
			full_timer3_count++;
			full_vbatt_n = oppo_short_c_batt_get_average_vbatt_mv(chip, false);
			if (full_timer3_count < chip->short_c_batt.full_timer3)
				short_debug("full_timer3_count[%d], full_vbatt_n[%d]\n", full_timer3_count, full_vbatt_n);
			...
			if (full_timer3_count < chip->short_c_batt.full_timer3) {//timer3 not overflow
				if (chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__FULL_ERR_CODE2) {
					short_debug("errcode[2] was set, waiting for timer3 overflow\n");
					return;
				}
				if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) <= DELTA_TEMP_THRESHOLD) {
					if (full_vbatt_n > 0 && (full_vbatt1 - full_vbatt_n) > chip->short_c_batt.full_delta_vbatt1_mv
							&& (chip->short_c_batt.batt_chging_cycles < chip->short_c_batt.batt_chging_cycle_threshold)) {
						//set err_code2;
				...
				} else if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
					...
					chg_err("exit_check, temperature[%d]\n", chip->temperature);
					short_debug("exit_check, T1[%d], T2[%d]\n", full_temp1, chip->temperature);
					exit_check_status = true;
					return;
				}
			} else {//timer3 overflow
				if (full_timer3_count == chip->short_c_batt.full_timer3) {
					if (chip->temperature <= COOL_TEMP_THRESHOLD) {//2~12
						if (chip->short_c_batt.dyna1_low_avg_dv_mv != INVALID_DATA
								&& chip->short_c_batt.dyna1_low_avg_dv_mv >= 0) {
							if (full_vbatt_n > 0 && (full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna1_low_avg_dv_mv
						...
						if (chip->short_c_batt.dyna1_high_avg_dv_mv != INVALID_DATA
							...
							if (full_vbatt_n > 0 && (full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna1_high_avg_dv_mv
									> chip->short_c_batt.dyna1_delta_dv_mv)) {
								//set err_code4;
								short_debug("set err_code[%d]\n", SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4);
								chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4;
				...
				full_timer3_count = chip->short_c_batt.full_timer3 + 1;

				/* start timer4 */
				full_timer4_count++;
				short_debug("full_timer4_count[%d], full_vbatt_n[%d]\n", full_timer4_count, full_vbatt_n);
				if (full_timer4_count < chip->short_c_batt.full_timer4) {//timer4 not overflow
					if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) <= DELTA_TEMP_THRESHOLD) {
						if (full_vbatt_n > 0 && (full_vbatt1 - full_vbatt_n) > chip->short_c_batt.full_delta_vbatt2_mv
								&& (chip->short_c_batt.batt_chging_cycles < chip->short_c_batt.batt_chging_cycle_threshold)) {
							//set err_code3;
				...
				} else {//timer4 overflow
					if (chip->temperature <= COOL_TEMP_THRESHOLD) {//2~12
						if (chip->short_c_batt.dyna2_low_avg_dv_mv != INVALID_DATA
								&& chip->short_c_batt.dyna2_low_avg_dv_mv >= 0) {
							if (full_vbatt_n > 0 && (full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna2_low_avg_dv_mv
						...
						if (chip->short_c_batt.dyna2_high_avg_dv_mv != INVALID_DATA
							...
							if (full_vbatt_n > 0 && (full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna2_high_avg_dv_mv
									> chip->short_c_batt.dyna2_delta_dv_mv)) {
								//set err_code5;
								short_debug("set err_code[%d]\n", SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5);
								chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5;
...
int oppo_short_c_batt_feature_status_init(void)
{
	return SHORT_C_BATT_FEATURE_STATUS__ON;
}
