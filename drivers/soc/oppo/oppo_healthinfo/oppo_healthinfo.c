...
/*******************************   sync wait read   ***************************************************/
struct wait_para fsync_wait_para = {100, 500, 0, 0, 0, 0, 0, 0, 0, 0, 0};

void fsync_time_cal(int fg, u64 fsync_time)
{
        u64 delta_ms = fsync_time;
        if (!oppo_healthinfo_switch)
                return;

        fsync_wait_para.total_ms += delta_ms;
        fsync_wait_para.total_cnt++;
        if (fg) {
                fsync_wait_para.fg_total_ms += delta_ms;
                fsync_wait_para.fg_total_cnt++;
        }
        if (delta_ms >= fsync_wait_para.high_thresh_ms) {
                fsync_wait_para.high_cnt++;
                if (fg) {
                        fsync_wait_para.fg_high_cnt++;
                }
        } else if (delta_ms >= fsync_wait_para.low_thresh_ms) {
                fsync_wait_para.low_cnt++;
                if (fg) {
                        fsync_wait_para.fg_low_cnt++;
                }
        }
        if (fg & (delta_ms > fsync_wait_para.delta_ms))
                fsync_wait_para.delta_ms = delta_ms;
        return;
}
...
static ssize_t fsync_wait_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        ...
        len = sprintf(page, "fsync_low_thresh_ms: %d\n""fsync_high_thresh_ms: %d\n"
                ...
                "fsync_fg_max_ms: %lld\n",
                fsync_wait_para.low_thresh_ms, fsync_wait_para.high_thresh_ms,
                fsync_wait_para.low_cnt, fsync_wait_para.high_cnt,
                fsync_wait_para.total_ms, fsync_wait_para.total_cnt,
                fsync_wait_para.fg_low_cnt, fsync_wait_para.fg_high_cnt,
                fsync_wait_para.fg_total_ms, fsync_wait_para.fg_total_cnt,
                fsync_wait_para.delta_ms);
...
static int __init oppo_healthinfo_init(void)
{
	...
	if(!pentry) {
		health_err("create fsync_wait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
